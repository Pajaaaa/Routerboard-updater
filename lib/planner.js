...
async function plan(info, o) {
  ...
  for (const hop of hops) {
    ...
    if (hasFlashDir) {
      ...
      // pojistka z praxe: 16 MB kus na velmi staré v6 (< 6.45, starý RouterBOOT) — doložený pád (DISC Lite5 6.43 se po upgradu nevrátil)
      if (smallFlash && fromV.major === 6 && V.cmpVersion(fromV, '6.45') < 0 && !options.allow_small_flash && !settings.allow_old_small_flash && !hop.split && hop === hops[0]) {
        blockers.push(`16 MB flash na startovní verzi ${hop.from} (pod 6.45): instalace z RAM se u takhle starých kusů neosvědčila (DISC Lite5 na 6.43 se po restartu nevrátil) — upgraduj ručně po krocích s Netinstallem po ruce, nebo povol v nastavení („16 MB flash na v6 pod 6.45“) či v pokročilém dialogu jobu`);
      }
      ...
      if (info.free_hdd && hop.needBytes > info.free_hdd) {
        ...
        if (routerMode) warnings.push(msg + ' — o stažení rozhodne updater routeru (při nedostatku místa skončí chybou bez restartu)');
          ...
          const allowed = options.allow_small_flash || (settings.allow_v7_small_flash && !ancient) || (ancient && settings.allow_old_small_flash);
