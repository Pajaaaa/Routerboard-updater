    ...
    if (hasFlashDir) {
      ...
      if (info.free_hdd && hop.needBytes > info.free_hdd) {
        ...
        if (routerMode) warnings.push(msg + ' — o stažení rozhodne updater routeru (při nedostatku místa skončí chybou bez restartu)');
          ...
          // globální povolení v nastavení je míněno pro v7 (tam se odložení do RAM osvědčilo); u cíle v6 jen výslovné povolení v jobu
          const allowed = options.allow_small_flash || (settings.allow_v7_small_flash && hv.major >= 7);
          if (allowed) { if (hop === hops[0]) warnings.push(tight + ' (POVOLENO, na vlastní riziko)'); }
          else if (hop === hops[0]) blockers.push(tight + (hv.major >= 7 ? ' (blokováno; povolit lze v Nastavení nebo v jobu)' : ' (blokováno; u cíle v6 lze povolit jen v pokročilém dialogu jobu)'));
