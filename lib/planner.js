...
function linkNotes(info, settings, notes, warnings) {
  ...
  for (const w of links.w60g || []) {
    ...
    for (const x of w.stations || []) {
      if (x.signal !== null && x.signal !== undefined && x.signal < minSig60) warnings.push(`slabé 60 GHz spojení: ${w.iface} → ${x.mac} kvalita ${x.signal} % (limit ${minSig60})${x.rssi !== null ? `, RSSI ${x.rssi} dBm` : ''}`);
      if (x.mcs !== null && x.mcs !== undefined && x.mcs > 0 && x.mcs < 2) warnings.push(`nízká rychlost 60 GHz: ${w.iface} → ${x.mac} MCS ${x.mcs}`);
      if (x.per !== null && x.per !== undefined && x.per > maxPer) warnings.push(`chybovost 60 GHz: ${w.iface} → ${x.mac} tx-packet-error-rate ${x.per} % (limit ${maxPer})`);
    }
