...
function linkNotes(info, settings, notes, warnings, blockers = [], options = {}) {
  ...
  for (const s of links.stations || []) {
    ...
    if (s.apDev && (!s.country || !s.apDev.country || !s.freqMode || !s.apDev.freqMode)) {
      ...
      const diff = [];
      if (norm(s.country) !== norm(s.apDev.country)) diff.push(`country „${s.country || '?'}“ × sektor „${s.apDev.country || '?'}“`);
      if (norm(s.freqMode) !== norm(s.apDev.freqMode)) diff.push(`frequency-mode „${s.freqMode || '?'}“ × sektor „${s.apDev.freqMode || '?'}“`);
      if (diff.length) {
      ...
      } else notes.push(`stanice ${s.iface}: country „${s.country || '?'}“ a frequency-mode „${s.freqMode || '?'}“ odpovídají sektoru ${s.apDev.identity || s.apDev.host}`);
