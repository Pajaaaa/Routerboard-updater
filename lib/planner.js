...
async function plan(info, o) {
  ...
  // málo RAM pro v7 (RB711, RB750, hAP lite: 32 MB) → místo blokování cílit na poslední v6 (MikroTik: v7 ne pod 64 MB RAM)
  if (target && /^7\./.test(target) && info.total_mem && info.total_mem < 60 * MB && !(settings.allow_v7_low_ram || options.allow_v7_low_ram)) {
    const v6 = targetFor('v6-long-term', latest);
    if (v6) { target = v6; out.track = 'v6-long-term'; warnings.push(`jen ${(info.total_mem / MB).toFixed(0)} MB RAM — v7 se na tomhle HW neprovozuje (MikroTik: min. 64 MB), cíl přepnut na v6 long-term ${v6}; přepni zařízení na track v6-long-term`); }
  }
  ...
  if (info.free_mem && info.free_mem < minFree) blockers.push(`málo volné RAM: ${(info.free_mem / MB).toFixed(1)} MB (min. ${(minFree / MB).toFixed(1)})`);
