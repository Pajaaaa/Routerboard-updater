...
async function plan(info, o) {
  ...
  if (dm && dm.flagged) {
    if (options.ignore_flagged) warnings.push('device-mode je FLAGGED (RouterOS detekoval možnou kompromitaci) — u zařízení je zaškrtnuto „prověřeno, upgradovat i tak“; bez fyzického odblokování zůstane omezení (scheduler, VPN, proxy…) i po upgradu');
