    ...
    if (hasFlashDir) {
      ...
      if (info.free_hdd && hop.needBytes > info.free_hdd) {
        ...
        if (routerMode) warnings.push(msg + ' — o stažení rozhodne updater routeru (při nedostatku místa skončí chybou bez restartu)');
          ...
          if (settings.allow_v7_small_flash || options.allow_small_flash) { if (hop === hops[0]) warnings.push(tight + ' (POVOLENO nastavením, na vlastní riziko)'); }
          else if (hop === hops[0]) blockers.push(tight + ' (blokováno; povolit lze v Nastavení nebo v jobu)');
