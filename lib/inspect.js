'use strict';
// MikroTik upgrader — sken zařízení — pouze čtení.
...
async function inspect(c, { full = false } = {}) {
  ...
  // přetížený kus (CPU ≥ 90 %, typicky 32 MB RAM na v7): každý get trvá 3–12 s → limity příkazů rovnou ×3, ať zbytek sběru nepadá na timeoutech
  if (num(r['cpu-load']) >= 90 && typeof c.markSlow === 'function' && c.slowFactor < 3) c.markSlow(`CPU ${num(r['cpu-load'])} %`, 3);
  ...
  if (full) {
    ...
    // Jak daleko soused je: LLDP rámce chodí na vyhrazenou multicast adresu, kterou switche podle normy nepřeposílají — soused nalezený
    // přes LLDP tedy visí přímo na kabelu. MNDP a CDP se šíří celou broadcast doménou, takový soused může být o několik switchů dál.
    // ROS 7 to má v poli discovered-by; ROS 6 ho nezná, tam je náhradou system-caps (plní se jen z LLDP). Extra dotaz zvlášť, ať
    // neexistující pole neshodí celý seznam sousedů (list() zahazuje záznam, u kterého kterékoli pole selže).
    const v7nb = /^7\./.test(String(info.version || ''));
    let nbx = [];
    try { nbx = (await c.list('/ip neighbor', v7nb ? ['mac-address', 'interface-name', 'discovered-by'] : ['mac-address', 'interface-name', 'system-caps'])) || []; } catch { nbx = []; }
    const nbxByMac = new Map(nbx.filter(x => x['mac-address']).map(x => [String(x['mac-address']).toUpperCase(), x]));
    const nbDirect = (mac) => {
      const x = nbxByMac.get(String(mac || '').toUpperCase());
      if (!x) return null; // nevíme (staré ROS, dotaz neprošel)
      return v7nb ? /lldp/i.test(x['discovered-by'] || '') : !!String(x['system-caps'] || '').trim();
    };
    info.neighbors = nb.map(n => ({ iface: n.interface, address: n.address, identity: n.identity, board: n.board, mac: n['mac-address'], version: n.version,
      lldp: nbDirect(n['mac-address']), port: (nbxByMac.get(String(n['mac-address'] || '').toUpperCase()) || {})['interface-name'] || '' }));
    if (info.uplink) {
      ...
      info.uplink.neighbors_on_iface = info.neighbors.filter(n => info.uplink.iface && ifs(n).includes(info.uplink.iface)).map(n => ({ address: n.address, identity: n.identity, board: n.board, mac: n.mac, lldp: n.lldp, port: n.port }));
    }
    ...
    // Switch s VLAN filtrováním a adresou na bridgi: novější RouterOS už procesor nepřidá do jeho VLAN netagovaně sám (dřív to plynulo
    // z pvid bridge). Po upgradu zařízení dál přepíná, ale nikdo se na něj nedostane — 9.9.2026 Kenny3-Switch, 10.9.2026 switch Libuše,
    // obojí CRS328 se stejnou konfigurací. Zjišťuje se stav PŘED upgradem, aby to plánovač mohl zablokovat.
    info.vlan_mgmt = [];
    try {
      const brs = (await c.list('/interface bridge', ['name', 'vlan-filtering', 'pvid'])) || [];
      if (brs.some(b => bool(b['vlan-filtering']))) {
        let bv = (await c.list('/interface bridge vlan', ['bridge', 'vlan-ids', 'current-untagged'])) || [];
        if (!bv.length) bv = ((await c.list('/interface bridge vlan', ['bridge', 'vlan-ids', 'untagged'])) || []).map(x => ({ ...x, 'current-untagged': x.untagged }));
        const vifs = (await c.list('/interface vlan', ['name', 'interface', 'vlan-id', 'disabled'])) || [];
        info.vlan_mgmt = vlanMgmtRisk(brs, info.addresses, bv, vifs);
      }
    } catch { /* starší RouterOS bez některého pole — nehodnotíme, ať sken nespadne */ }
    flags.vlan_mgmt = info.vlan_mgmt.length ? info.vlan_mgmt : null;
...
async function inspectLinks(c, major) {
  ...
  const wl = (await c.list('/interface wireless', ['name', 'mode', 'wireless-protocol', 'band', 'ssid', 'running', 'disabled', 'mac-address', 'country', 'frequency-mode', 'frequency', 'scan-list', 'channel-width', 'installation'])) || [];
  ...
  for (const w of wl) {
    ...
    const base = { iface: w.name, mode: w.mode, protocol: w['wireless-protocol'] || '', band: w.band || '', ssid: w.ssid || '', running: w.running === 'true', mac: w['mac-address'] || '', country: w.country || '', installation: w.installation || '', freqMode: w['frequency-mode'] || '', frequency: w.frequency || '', scanList: w['scan-list'] || '', channelWidth: w['channel-width'] || '' };
  ...
  if (major >= 7) {
    const wf = (await c.list('/interface wifi', ['name', 'configuration.mode', 'configuration.ssid', 'running', 'disabled', 'mac-address', 'configuration.country', 'configuration.installation', 'channel.frequency', 'channel.band', 'channel.width'])) || [];
    ...
    for (const w of wf) {
      ...
      // wifi (ax) ovladač nemá frequency-mode; regulaci určuje configuration.country (+ installation indoor/outdoor), kanál channel.frequency
      const base = { iface: w.name, mode: w['configuration.mode'] || '', ssid: w['configuration.ssid'] || '', running: w.running === 'true', mac: w['mac-address'] || '', driver: 'wifi', country: w['configuration.country'] || '', installation: w['configuration.installation'] || '', freqMode: '', frequency: w['channel.frequency'] || '', band: w['channel.band'] || '', channelWidth: w['channel.width'] || '' };
      // wifi stanice patří mezi stanice (kontrola country vs sektor, čekání na návrat k AP) — v registration-table je u stanice řádek s MAC AP
      if (/^station/.test(base.mode)) links.stations.push({ ...base, protocol: '802.11', ap: mine[0] || null });
      else links.wifi.push({ ...base, clients: mine });
    }
  }
  // --- CAP / CAPsMAN (legacy = balíček wireless; wifi CAPsMAN = bundle; navzájem nekompatibilní) ---
  // „current-caps-man-address“ přes get neexistuje (jen ve výpisu print, a v 7.23 ani tam) → registrace se pozná podle toho, že rozhraní
  // řízená CAPsMAN (cap interfaces) zase běží; ukládá se jejich název, MAC a stav před restartem
  const cap = await c.kv('/interface wireless cap', ['enabled', 'caps-man-addresses', 'interfaces']);
  const radiosOf = (names, pool) => names.map(n => { const w = pool.find(x => x.name === n); return { name: n, mac: w ? (w['mac-address'] || '') : '', running: !!w && w.running === 'true' }; });
  if (cap.enabled === 'true') links.cap = { kind: 'legacy', manager: '', configured: cap['caps-man-addresses'] || '', radios: radiosOf(String(cap.interfaces || '').split(/[;,]/).map(x => x.trim()).filter(Boolean), wl) };
  if (major >= 7) {
    const wcap = await c.kv('/interface wifi cap', ['enabled', 'caps-man-addresses']);
    if (wcap.enabled === 'true') {
      // wifi CAP nemá seznam rozhraní — bere se každé wifi rozhraní, které běželo (CAPsMAN je po registraci nastaví a spustí)
      const wf = (await c.list('/interface wifi', ['name', 'mac-address', 'running', 'disabled'])) || [];
      links.cap = { kind: 'wifi', manager: '', configured: wcap['caps-man-addresses'] || '', radios: radiosOf(wf.filter(x => x.disabled !== 'true').map(x => x.name), wf) };
    }
  }
...
/**
 * Switch s VLAN filtrováním, který po upgradu ztratí management: adresa je na bridgi, ale bridge (procesor) není netagovaným
 * členem VLAN, kterou má jako pvid. Starší RouterOS členství CPU odvodil z pvid, novější (7.13+) ne — zařízení pak dál přepíná,
 * ale nikdo se na něj nedostane (9.9.2026 Kenny3-Switch, 10.9.2026 switch Libuše, oba CRS328).
 * Vrací [{ bridge, pvid, address, vlan_iface }] — prázdné pole = v pořádku.
 */
function vlanMgmtRisk(bridges, addresses, bridgeVlans, vlanIfaces) {
  const yes = (v) => /^(true|yes)$/i.test(String(v == null ? '' : v).trim());
  const parts = (v) => String(v == null ? '' : v).split(/[;,]/).map(x => x.trim()).filter(Boolean); // RouterOS vrací seznamy se středníkem
  const hasVid = (v, vid) => parts(v).some(t => { const m = t.match(/^(\d+)-(\d+)$/); return m ? (+vid >= +m[1] && +vid <= +m[2]) : t === String(vid); });
  const vlanBrs = (bridges || []).filter(b => yes(b['vlan-filtering']));
  const out = [], seen = new Set();
  for (const a of addresses || []) {
    if (yes(a.disabled)) continue;
    const b = vlanBrs.find(x => x.name === a.interface);
    if (!b) continue;
    const pvid = String(b.pvid || '1');
    const key = b.name + '/' + pvid;
    if (seen.has(key)) continue;
    const rows = (bridgeVlans || []).filter(v => v.bridge === b.name && hasVid(v['vlan-ids'], pvid));
    if (!rows.length) continue; // pro pvid není žádný záznam → členství CPU si RouterOS řeší sám
    if (rows.some(v => parts(v['current-untagged']).includes(b.name))) continue; // procesor je ve své VLAN netagovaně → v pořádku
    seen.add(key);
    const vif = (vlanIfaces || []).find(v => v.interface === b.name && String(v['vlan-id']) === pvid && !yes(v.disabled));
    out.push({ bridge: b.name, pvid, address: String(a.address || ''), vlan_iface: vif ? vif.name : '' });
  }
  return out;
}
...
module.exports = { inspect, vlanMgmtRisk, inspectLinks, inspectPoeWatchdogs, inspectRebootScripts, inspectUserPolicy, configCounts, logSymptoms, toDeviceFields, parseUptime };
