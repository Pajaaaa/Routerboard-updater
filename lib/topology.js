...
function suggestParent(dev, all) {
  ...
  for (const { d, k } of hits) {
    ...
    if (cands.length === 1 && cands[0].id === dev.id) return { id: d.id, name: devLabel(d), via: `PoE ${k.iface || ''}`.trim(), address: d.host, identity: d.identity || '', src: 'poe' };
  }
  ...
  if (cap && cap.manager) {
    ...
    if (c) return { id: c.id, name: devLabel(c), via: 'CAPsMAN', address: host, identity: c.identity || '', src: 'capsman' };
  }
  ...
  return cand ? { id: cand.id, name: devLabel(cand), via: up.iface, address: n.address, identity: n.identity, src: 'gateway' } : { id: 0, name: '', via: up.iface, address: n.address, identity: n.identity, src: 'gateway' };
}
...
function makeIndex(all) {
  ...
  for (const d of all) {
    // výchozí adresy 192.168.88.x má spousta kusů → do indexu nepatří (falešné shody brány/PoE)
    for (const a of [d.host, ...((d.flags && Array.isArray(d.flags.ip_addresses)) ? d.flags.ip_addresses : [])]) if (a && !/^192\.168\.88\./.test(a) && !ip.has(a)) ip.set(a, d);
    ...
    for (const k of (d.flags && d.flags.poe_children) || []) {
      if (k.address && !/^192\.168\.88\./.test(k.address)) { const arr = poeAddr.get(k.address) || []; arr.push({ d, k }); poeAddr.set(k.address, arr); }
...
function radioParent(dev, all) {
  ...
  for (const s of l.stations || []) {
    ...
    if (ap) return { id: ap.id, name: devLabel(ap), via: `${s.iface} → AP`, address: (s.ap && s.ap.mac) || s.mac, identity: ap.identity || '' };
  }
  ...
  for (const w of l.wifi || []) {
    ...
    if (ap) return { id: ap.id, name: devLabel(ap), via: `${w.iface} → AP`, address: (peer && peer.mac) || w.ssid || '', identity: ap.identity || '' };
  }
  for (const w of l.w60g || []) {
    if (!/^station/.test(w.mode || '') || !w.remote) continue;
    const ap = deviceByRadioMac(w.remote, all, dev.id);
    if (ap) return { id: ap.id, name: devLabel(ap), via: `${w.iface} 60 GHz → AP`, address: w.remote, identity: ap.identity || '' };
  }
  return null;
}
/**
