...
function suggestParent(dev, all) {
  ...
  const poe = all.find(d => d.id !== dev.id && d.flags && (d.flags.poe_children || []).some(k => (k.address && k.address === dev.host) || (k.identity && dev.identity && k.identity === dev.identity)));
  if (poe) { const k = (poe.flags.poe_children || []).find(k => k.address === dev.host || k.identity === dev.identity) || {}; return { id: poe.id, name: poe.name || poe.identity || poe.host, via: `PoE ${k.iface || ''}`.trim(), address: poe.host, identity: poe.identity || '', src: 'poe' }; }
