...
function suggestParent(dev, all) {
  ...
  // bere se jen port, na kterém je vidět jediný soused: když jich je na PoE portu víc, visí za ním switch nebo spoj a napájený je jen ten první (nepoznáme který)
  const isMe = k => (k.address && k.address === dev.host) || (k.identity && dev.identity && k.identity === dev.identity);
  for (const d of all) {
    if (d.id === dev.id || !d.flags) continue;
    const kids = d.flags.poe_children || [];
    const k = kids.find(isMe);
    if (!k) continue;
    if (kids.filter(x => x.iface === k.iface).length > 1) continue;
    return { id: d.id, name: d.name || d.identity || d.host, via: `PoE ${k.iface || ''}`.trim(), address: d.host, identity: d.identity || '', src: 'poe' };
  }
