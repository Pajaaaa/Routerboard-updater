...
function autoParent(devId) {
  ...
  if (dev.parent_id) {
    ...
    const cur = all.find(d => d.id === dev.parent_id);
    const gw = dev.flags && dev.flags.uplink && dev.flags.uplink.gateway;
    const weak = dev.parent_src === 'gateway' || (!dev.parent_src && cur && gw && cur.host === gw);
    if (!weak) return 0;
  }
