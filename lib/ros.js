...
class RosClient {
  ...
  static isReadOnly(cmd) {
    const c = String(cmd || '');
    if (/(^|[\s;{[])(set|add|remove|enable|disable|move|reset|reboot|shutdown|install|uninstall|downgrade|upgrade|fetch|save|load|import|export|regenerate|power-cycle|unset|scheduler)\b/i.test(c)) return false;
