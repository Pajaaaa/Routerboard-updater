...
class RosClient {
  ...
  /** Spustí příkaz; když mezitím spadne SSH a jde o čtecí příkaz, naváže spojení znovu a zkusí to ještě dvakrát. */
  async exec(cmd, opts = {}) {
    const tries = opts.noRetry || !RosClient.isReadOnly(cmd) ? 1 : 3;
    let last;
    for (let i = 1; i <= tries; i++) {
      try { return await this._execOnce(cmd, opts); }
      catch (e) {
        last = e;
        const lost = RosClient.isConnLost(e.message) || !this.conn || this.closed;
        if (i >= tries || !lost || this.disposed) throw e; // zavřeného klienta neoživujeme
  ...
  /** list() pro pomalé výpisy (typicky /file po prvním startu nové verze na 16 MB flash trvá i přes 30 s): delší limit a při timeoutu ještě 2 pokusy */
  async listPatient(menu, fields, opts = {}) {
    let last;
    for (let i = 0; i < 3; i++) {
      try { return await this.list(menu, fields, { timeoutMs: 60000, ...opts }); }
      catch (e) { last = e; if (!/timeout příkazu/.test(e.message) || this.closed) throw e; }
    }
    throw last;
  }
  /**
