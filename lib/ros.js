...
class RosClient {
  ...
  async _ping() {
    try { return /^1/.test(String(await this._execOnce(':put 1', { timeoutMs: 8000 })).trim()); } catch { return false; }
  }
  ...
  async exec(cmd, opts = {}) {
    ...
    let last, timeoutMs = opts.timeoutMs || 30000;
    for (let i = 1; i <= tries; i++) {
      try { return await this._execOnce(cmd, { ...opts, timeoutMs }); }
        ...
        if (!lost && /timeout příkazu/.test(e.message)) {
          if (await this._ping()) { timeoutMs = Math.min(timeoutMs * 2, 180000); continue; }
          lost = true;
        }
        if (!lost) throw e;
