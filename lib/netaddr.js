...
/** smí se na tuhle adresu sahat (sken, přidání zařízení)? Jen IP literál (bez hostname, ať se allowlist nedá obejít přes DNS) v seznamu; '*' = vše */
function hostAllowed(host, list) {
  const h = String(host || '').trim();
  if (String(list || '').trim() === '*') return !!h;
  if (!net.isIP(h)) return false;
  return ipInList(h, list);
}
module.exports = { ipInList, hostAllowed };
