...
class Scanner {
  ...
  async scanAll(ids, meta = {}) {
    ...
    if (meta.tag === 'check' && meta.ownerId) this.checkRuns.set(meta.ownerId, run);
    const track = (finished) => { if (meta.tag === 'check' && meta.ownerId) this.checks.set(meta.ownerId, { done, total: all.length, startedAt, finishedAt: finished ? Date.now() : 0, cancelled: run.cancel }); };
    ...
    track(true);
    if (meta.tag === 'check' && meta.ownerId && this.checkRuns.get(meta.ownerId) === run) this.checkRuns.delete(meta.ownerId);
    this.bus.emit('event', { type: 'scan-done', count: done, cancelled: run.cancel, ...meta, ids: meta.tag ? all : undefined });
    return list.length;
  }
}
