...
class Scanner {
  constructor(runner, bus) {
    ...
    this.checks = new Map(); // ownerId -> { done, total, startedAt, finishedAt } — průběh hromadné kontroly (tag 'check') pro progress bar v UI
    this.timer = null;
  }
  ...
  async scanAll(ids, meta = {}) {
    ...
    const track = (finished) => { if (meta.tag === 'check' && meta.ownerId) this.checks.set(meta.ownerId, { done, total: all.length, startedAt, finishedAt: finished ? Date.now() : 0 }); };
