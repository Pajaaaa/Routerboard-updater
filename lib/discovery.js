...
class Discovery {
  ...
  setResult(o) {
    ...
    this.state = { ownerId: o.ownerId || 0, startedAt: Date.now(), finishedAt: Date.now(), total: 0, done: 0, open: 0, found: [], added: 0, existing: 0, foreign: [...(o.foreign || [])], authFailed: [], notRouterOS: [], errors: [...(o.errors || [])] };
    this.bus.emit('event', { type: 'discovery-done', state: this.state });
    return this.state;
  }
  /**
  ...
  async run(o) {
    ...
    const st = this.state = { ownerId: o.ownerId || 0, startedAt: Date.now(), finishedAt: 0, total: hosts.length, done: 0, open: 0, found: [], added: 0, existing: 0, foreign: [...(o.foreign || [])], authFailed: [], notRouterOS: [], errors: [...(o.errors || [])] };
      ...
      while (queue.length) {
        ...
        try {
          ...
          if (ex) {
            ...
            if (ex.owner_id && ex.owner_id !== (o.ownerId || 0)) { const ow = db.getUser(ex.owner_id); st.foreign.push(`${host}: má u sebe ${ow ? ow.name : 'jiný uživatel'} — každé zařízení může mít jen jednoho vlastníka, o předání požádej jeho nebo správce`); }
          ...
          for (const cr of credList) {
            ...
            try {
              ...
              const devId = db.insertDevice({ host, port: hostPort, username: cr.username, password_enc: encrypt(cr.password), name: (ent && ent.name) || '', group_name: (ent && ent.group_name) || o.group_name || '', track: o.track || 'v7-stable', owner_id: o.ownerId || 0 });
