'use strict';
// MikroTik upgrader — hledání zařízení v rozsazích a jejich zakládání.
...
const { hostAllowed } = require('./netaddr');
...
class Discovery {
  ...
  prepare(o) {
    ...
    // allowlist adres (MTU_SCAN_ALLOW): server nesmí sloužit jako skener nebo slovníkový útočník na cizí sítě
    const bad = hosts.filter(h => !hostAllowed(h, cfg.scanAllow));
    if (bad.length) throw new Error(`adresy mimo povolené rozsahy (${cfg.scanAllow}): ${bad.slice(0, 5).join(', ')}${bad.length > 5 ? ` … +${bad.length - 5}` : ''} — skenovat a přidávat jde jen adresy vlastní sítě`);
    if (ranges.length && !(o.creds || []).length) throw new Error('k rozsahu zadej aspoň jeden login (uživatel heslo)');
    return hosts;
  }
  _blank(o, hosts) {
    return { ownerId: o.ownerId || 0, label: o.label || '', queued: false, position: 0, startedAt: 0, finishedAt: 0, total: hosts.length, done: 0, open: 0, found: [], added: 0, existing: 0, foreign: [...(o.foreign || [])], authFailed: [], notRouterOS: [], winboxOnly: [], takeover: [...(o.takeover || [])], errors: [...(o.errors || [])] };
  }
  ...
  async _exec({ o, hosts, st }) {
    ...
    const settings = db.getSettings(o.ownerId); // nastavení uživatele, který sken spustil
    ...
    const worker = async () => {
      while (queue.length) {
        ...
        try {
          ...
          if (!(await probeTcp(host, hostPort, 1500))) {
            // SSH zavřené: když odpovídá Winbox (8291), je to skoro jistě RouterOS bez SSH — nástroj ho neumí spravovat, důrazně upozornit
            if (await probeTcp(host, 8291, 1500)) { st.winboxOnly.push(host + (ent ? ` (${ent.name || ent.username || ''})`.replace(' ()', '') : '')); continue; }
            if (ent) st.errors.push(`${host}: port ${hostPort} neodpovídá`);
            continue;
          }
          st.open++;
          ...
          if (ex) {
            ...
            if (ex.owner_id && ex.owner_id !== ownerId) { const ow = db.getUser(ex.owner_id); st.foreign.push(`${host}: má u sebe ${ow ? (ow.userdb_nick || ow.name) : 'jiný uživatel'} — každé zařízení může mít jen jednoho vlastníka, o předání požádej jeho nebo správce`); }
          ...
          for (const cr of credList) {
            // víc než 2 pokusy během minuty by mohly spustit brute-force blacklist na zařízení → rozestup
            ...
            try {
              ...
              const devId = db.insertDevice({ host, port: hostPort, username: cr.username, password_enc: encrypt(cr.password), name: (ent && ent.name) || '', group_name: (ent && ent.group_name) || o.group_name || '', track: o.track || settings.default_track || 'v7-stable', owner_id: (ent && ent.ownerId) || o.ownerId || 0 });
    ...
    try { await Promise.all(Array.from({ length: Math.max(1, Math.min(16, o.parallel || 12)) }, worker)); }
