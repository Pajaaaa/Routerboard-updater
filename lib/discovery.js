...
class Discovery {
  ...
  prepare(o) {
    ...
    const entries = o.entries || []; // seznam zařízení s vlastním loginem: {host, port?, username, password}
  ...
  async run(o) {
    ...
    const st = this.state = { ownerId: o.ownerId || 0, startedAt: Date.now(), finishedAt: 0, total: hosts.length, done: 0, open: 0, found: [], added: 0, existing: 0, foreign: [], authFailed: [], notRouterOS: [], errors: [] };
        ...
        try {
          ...
          for (const cr of credList) {
            ...
            try {
              ...
              const devId = db.insertDevice({ host, port: hostPort, username: cr.username, password_enc: encrypt(cr.password), name: (ent && ent.name) || '', group_name: o.group_name || '', track: o.track || 'v7-stable', owner_id: o.ownerId || 0 });
