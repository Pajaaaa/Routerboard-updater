...
/** Kompletní seznam zařízení správce (APčka + členové pod nimi, přes všechny jeho oblasti) včetně loginů: [{ ip, login, password, name, note, member, userId, ap, apId, area, areaId }] */
async function devicesFor(who) {
  ...
  for (const a of w.areas) for (const ap of a.aps) {
    for (const d of await devicesForAp(ap.id)) {
      ...
      else missing.push({ ip: d.ip, name: d.name || d.note || '', member: d.member, ap: ap.name, area: a.name });
    }
  }
