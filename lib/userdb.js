...
async function devicesFor(who, { apIds = null, concurrency = 6 } = {}) {
  ...
  const ids = [];
  for (const a of w.areas) for (const ap of a.aps) if (!only || only.has(ap.id)) ids.push(ap.id);
  const r = await devicesForAps(ids, { concurrency });
  return { admin: w, ...r };
}

/**
 * Zařízení libovolných APček (podle id) včetně loginů — pro správce, který importuje celou síť.
 * Vrací { devices: [{ ip, login, password, name, note, member, userId, ap, apId, area, areaId, areaAdmins }], missing: [...] }
 */
async function devicesForAps(apIds, { concurrency = 6 } = {}) {
  const all = await areas();
  const byAp = new Map();
  for (const a of all) for (const ap of a.aps) byAp.set(ap.id, { ap, a });
  const todo = [];
  for (const id of [...new Set((apIds || []).map(Number))]) {
    const hit = byAp.get(id); if (!hit) continue;
    for (const d of await devicesForAp(id)) todo.push({ d, ap: hit.ap, a: hit.a });
  }
  ...
  const worker = async () => {
    while (i < todo.length) {
      ...
      const base = { name: d.name || '', note: d.note || '', member: d.member, userId: d.userId, ap: ap.name, apId: ap.id, area: a.name, areaId: a.id, areaAdmins: a.admins };
      if (c) devices.push({ ...c, ...base });
...
module.exports = { enabled, areas, whoIs, getCredentials, devicesForAp, devicesFor, devicesForAps, cfg };
