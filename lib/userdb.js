...
async function devicesByAp(force) {
  ...
  for (const typ of wanted) {
    ...
    for (const [ip, d] of Object.entries(z)) {
      ...
      list.push({ ip, name: String(d.hostname || '').trim(), note: String(d.popis || '').trim(), type: typ, typeName: types[typ] || String(typ) });
...
async function devicesFor(who) {
  ...
  for (const a of w.areas) for (const ap of a.aps) {
    for (const d of await devicesForAp(ap.id)) {
      ...
      if (c) devices.push({ ...c, name: d.name || '', note: d.note || '', type: d.type, typeName: d.typeName, ap: ap.name, apId: ap.id, area: a.name, areaId: a.id });
      else missing.push({ ip: d.ip, name: d.name || d.note || '', typeName: d.typeName, ap: ap.name, area: a.name });
    }
  }
  return { admin: w, devices, missing };
}

module.exports = { enabled, areas, whoIs, getCredentials, deviceTypes, devicesByAp, devicesForAp, devicesFor, cfg };
