 ...
 * což je jeden dotaz místo stovek → drží se v cache. MTU_USERDB_DEVICE_TYPES (výchozí "2") říká, které typy se importují.
 *
...
const cfg = {
  ...
  deviceTypes: String(process.env.MTU_USERDB_DEVICE_TYPES || '2').split(/[\s,;]+/).filter(Boolean),
...
let devCache = null; // { at, byAp: Map<apId, [{ip, name, note, type}]> }
/** Všechna síťová zařízení importovaných typů na všech APčkách, seskupená podle AP (jeden dotaz na typ, cache). */
async function devicesByAp(force) {
  if (!force && devCache && Date.now() - devCache.at < cfg.cacheMs) return devCache.byAp;
  const byAp = new Map();
  for (const typ of cfg.deviceTypes) {
    const r = await getJson(`${cfg.url}/monitoring/get-zarizeni?typ=${encodeURIComponent(typ)}`);
    ...
    for (const [ip, d] of Object.entries(z)) {
      if (!d || !d.Ap_id) continue;
      const list = byAp.get(Number(d.Ap_id)) || [];
      list.push({ ip, name: String(d.hostname || '').trim(), note: String(d.popis || '').trim(), type: Number(typ) });
      byAp.set(Number(d.Ap_id), list);
    }
  }
  devCache = { at: Date.now(), byAp };
  return byAp;
}
...
async function devicesFor(who) {
  ...
  for (const a of w.areas) for (const ap of a.aps) {
    for (const d of await devicesForAp(ap.id)) {
      ...
      if (c) devices.push({ ...c, name: d.name || '', note: d.note || '', ap: ap.name, apId: ap.id, area: a.name, areaId: a.id });
      else missing.push({ ip: d.ip, ap: ap.name, area: a.name });
    }
  }
  return { admin: w, devices, missing };
}

module.exports = { enabled, areas, whoIs, getCredentials, devicesByAp, devicesForAp, devicesFor, cfg };
