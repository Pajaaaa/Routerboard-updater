 ...
 * MTU_USERDB_AP_DEVICES_URL: šablona endpointu, který k APčku vrátí jeho zařízení (IP), např.
 * ".../device/list-by-ap/{ap}" — doplní se, až bude endpoint k dispozici; do té doby devicesForAp() vrací [].
 *
...
const cfg = {
  ...
  apDevicesUrl: process.env.MTU_USERDB_AP_DEVICES_URL || '',
...
/** Zařízení (IP) patřící k APčku. Endpoint zatím není k dispozici → [] (viz MTU_USERDB_AP_DEVICES_URL). */
async function devicesForAp(apId) {
  if (!cfg.apDevicesUrl) return [];
  const r = await getJson(cfg.apDevicesUrl.replace('{ap}', encodeURIComponent(apId)));
  return vals(r).map(d => (typeof d === 'string' ? { ip: d } : { ip: d.ip || d.adresa || d.address, name: d.jmeno || d.name || '' })).filter(d => d.ip);
}
...
async function devicesFor(who) {
  ...
  for (const a of w.areas) for (const ap of a.aps) {
    for (const d of await devicesForAp(ap.id)) {
      ...
      if (c) devices.push({ ...c, name: d.name || '', ap: ap.name, apId: ap.id, area: a.name, areaId: a.id });
