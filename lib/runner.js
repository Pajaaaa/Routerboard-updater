...
class Runner extends EventEmitter {
  ...
  async precheckAll(jobId, opt, { early = false } = {}) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        await this.enrichStationAps(info, L);
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, allow_v7: !!raw.allow_v7, ignore_flagged: !!raw.ignore_flagged, prev_bad_blocks: pf.bad_blocks } });
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 6; iter++) {
        ...
        if (!hardened) { hardened = true; await this.hardenServices(c, L, W, settings, !!opt.dry_run); await this.setupRemoteLogging(c, L, W, settings, !!opt.dry_run); await this.setupTimeSync(c, info, L, W, settings, !!opt.dry_run); }
        if (p.nothingToDo) {
        ...
        const waitSettings = slowBoot ? { ...settings, reboot_timeout_min: Math.max(Number(settings.reboot_timeout_min) || 15, 25) } : settings;
        if (slowBoot && waitSettings.reboot_timeout_min !== (Number(settings.reboot_timeout_min) || 15)) L('info', `na návrat čekám až ${waitSettings.reboot_timeout_min} min (velký skok / 16 MB flash / mipsbe: první start bývá pomalý)`);
  ...
  async enrichStationAps(info, L) {
    ...
    for (const s of st) {
      ...
      for (const d of all) {
        const lk = (d.flags && d.flags.links) || {};
        const a = [...(lk.aps || []), ...(lk.wifi || [])].find(x => up(x.mac) === up(s.ap.mac));
        if (!a) continue;
        s.apDev = { host: d.host, identity: d.identity || d.name || '', driver: a.driver || 'wireless', country: a.country || '', freqMode: a.freqMode || '', installation: a.installation || '', frequency: a.frequency || '', band: a.band || '', version: d.version || '' };
        ...
        try {
          ...
          if (raw && raw.managed !== 0) {
            ...
            try {
              await c.connect();
              // RouterOS syntaxe je „/menu get [find …] vlastnost“ → do kv jde menu a klíč „[find …] vlastnost“ (menu „/menu [find …]“ je syntax error)
              const sel = `[find mac-address=${s.ap.mac}]`;
              if (s.apDev.driver === 'wifi') { const ks = ['configuration.country', 'configuration.installation', 'channel.frequency', 'channel.band']; const kv = await c.kv('/interface wifi', ks.map(k => `${sel} ${k}`)); const g = (k) => kv[`${sel} ${k}`]; if (g('configuration.country') !== null) Object.assign(s.apDev, { country: g('configuration.country') || '', installation: g('configuration.installation') || '', frequency: g('channel.frequency') || '', band: g('channel.band') || '', live: true }); else L && L('info', `sektor ${s.apDev.identity}: country nejde přečíst živě (rozhraní s MAC ${s.ap.mac} nenalezeno)`); }
              else { const ks = ['country', 'frequency-mode', 'frequency', 'band']; const kv = await c.kv('/interface wireless', ks.map(k => `${sel} ${k}`)); const g = (k) => kv[`${sel} ${k}`]; if (g('country') !== null) Object.assign(s.apDev, { country: g('country') || '', freqMode: g('frequency-mode') || '', frequency: g('frequency') || '', band: g('band') || '', live: true }); else L && L('info', `sektor ${s.apDev.identity}: country nejde přečíst živě (rozhraní s MAC ${s.ap.mac} nenalezeno)`); }
