...
class Runner extends EventEmitter {
  ...
  async ensureDeviceMode(c, raw, info, L, W, setStep, creds, connect, settings, want = { mode: 'advanced', partitions: true }) {
    ...
    // Proto se pustí na routeru na pozadí přes :execute; že ho router přijal, se pozná podle zvýšeného attempt-count.
    ...
    if (BAD.test(out)) { W(`device-mode update router odmítl: ${out.trim().split('\n')[0]} — bez změny`); return { client: c, changed: false }; }
    await sleep(3000);
    const attAfter = await readAttempts();
    if (Number.isFinite(attAfter) && attAfter <= attBefore) {
      W(`device-mode: router požadavek nepřijal (attempt-count zůstal ${attAfter}) — studený restart nedělám, ${need.join(' ')} zůstává nezměněné`);
      return { client: c, changed: false };
    }
    L('info', `device-mode update běží na routeru a čeká na potvrzení${Number.isFinite(attAfter) ? ` (pokus ${attAfter}/3)` : ''} — teď studený restart přes PoE rodiče`);
    try { c.close(); } catch {}
