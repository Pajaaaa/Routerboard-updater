...
class RunnerPool extends EventEmitter {
  ...
  conflictFor(raw, self) {
    for (const r of this.runners.values()) {
      ...
      if (other && physicallyAdjacent(raw, other)) { const u = db.getUser(r.ownerId); return { name: other.name || other.identity || other.host, host: other.host, user: u ? u.name : '?', jobId: r.currentJobId }; }
    }
    return null;
  }
  ...
  async earlyPrecheck(jobId) {
    ...
    try {
      ...
      const reasons = db.getJobItems(jobId).filter(it => /^přeskočí se/.test(it.step || '')).map(it => `${it.dev_name || it.identity || it.host}: ${it.step.replace(/^přeskočí se: /, '')}`);
      const warns = db.getJobItems(jobId).filter(it => it.status === 'pending' && it.warnings && it.warnings.length).map(it => `${it.dev_name || it.identity || it.host}: ${it.warnings.join(' | ')}`);
      const summary = `${res.ready} připraveno, ${res.blocked} se přeskočí${res.warned ? `, ${res.warned} s upozorněním` : ''}`;
      const j2 = db.getJob(jobId);
  ...
  /** kompatibilita: dřív fronta na uživatele, teď se spouští rovnou */
  startOrQueue(jobId) { this.start(jobId); return { queued: false }; }
