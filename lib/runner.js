...
class RunnerPool extends EventEmitter {
  constructor() {
    super();
    this.runners = new Map(); // jobId -> Runner (jen běžící)
  ...
  tickScheduled() {
    ...
    for (const j of db.listJobs(200)) {
      const due = (j.status === 'scheduled' && j.options && j.options.start_at && j.options.start_at <= now) || (j.status === 'queued' && j.options && j.options.queued_at);
      ...
      catch (e) { db.addLog(j.id, 0, 0, 'warn', 'naplánované spuštění zatím nejde: ' + e.message + ' — zkusím znovu za chvíli'); }
    }
  }
}
