...
class Runner extends EventEmitter {
  ...
  async stageViaRouter(c, hop, info, L, setStep, W, connect) {
    ...
    if (/downloaded/i.test(st.status || '') && st['latest-version'] === hop.to) {
      ...
      catch (e) { return { ok: false, error: `router nezná kanál "${hop.channel}": ${e.message}`, files, client: c }; }
  ...
  conflictFor(raw, self) {
    for (const r of this.runners.values()) {
      ...
      if (other && physicallyAdjacent(raw, other)) {
        ...
        const u = db.getUser(r.ownerId); return { name: other.name || other.identity || other.host, host: other.host, user: u ? (u.userdb_nick || u.name) : '?', jobId: r.currentJobId };
      }
    }
    return null;
  }
