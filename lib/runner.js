      ...
      for (const d of all) {
        ...
        try {
          ...
          if (raw && raw.managed !== 0) {
            ...
            try {
              await c.connect();
              if (s.apDev.driver === 'wifi') { const kv = await c.kv(`/interface wifi [find mac-address=${s.ap.mac}]`, ['configuration.country', 'configuration.installation', 'channel.frequency', 'channel.band']); if (kv['configuration.country'] !== null) Object.assign(s.apDev, { country: kv['configuration.country'] || '', installation: kv['configuration.installation'] || '', frequency: kv['channel.frequency'] || '', band: kv['channel.band'] || '', live: true }); }
              else { const kv = await c.kv(`/interface wireless [find mac-address=${s.ap.mac}]`, ['country', 'frequency-mode', 'frequency', 'band']); if (kv.country !== null) Object.assign(s.apDev, { country: kv.country || '', freqMode: kv['frequency-mode'] || '', frequency: kv.frequency || '', band: kv.band || '', live: true }); }
