...
class Runner extends EventEmitter {
  ...
  async enrichStationAps(info, L) {
    ...
    for (const s of st) {
      ...
      for (const d of all) {
        const aps = (d.flags && d.flags.links && d.flags.links.aps) || [];
        const a = aps.find(x => up(x.mac) === up(s.ap.mac));
        if (!a) continue;
        s.apDev = { host: d.host, identity: d.identity || d.name || '', country: a.country || '', freqMode: a.freqMode || '', frequency: a.frequency || '', band: a.band || '', version: d.version || '' };
        ...
        try {
          ...
          if (raw && raw.managed !== 0) {
            ...
            try { await c.connect(); const kv = await c.kv(`/interface wireless [find mac-address=${s.ap.mac}]`, ['country', 'frequency-mode', 'frequency', 'band']); if (kv.country !== null) Object.assign(s.apDev, { country: kv.country || '', freqMode: kv['frequency-mode'] || '', frequency: kv.frequency || '', band: kv.band || '', live: true }); } finally { try { c.close(); } catch {} }
          }
