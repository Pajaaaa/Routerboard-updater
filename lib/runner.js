...
class Runner extends EventEmitter {
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 6; iter++) {
        ...
        if (info.identity && raw.identity && info.identity !== raw.identity) { L('error', `identita zařízení nesouhlasí: očekáváno "${raw.identity}", nalezeno "${info.identity}"`); return finish('failed', { error: 'identita zařízení nesouhlasí' }); }
        ...
        if (after.version !== hop.to) {
          L('error', `po restartu je verze ${after.version}, očekáváno ${hop.to} — upgrade se neprovedl (zkontroluj /log na zařízení)`);
  ...
  poeParentOf(raw) {
    ...
    for (const n of nbs.filter(n => n.lldp === true)) { const d = resolve(n); if (hasPoe(d)) return db.getDeviceRaw(d.id); }
    ...
    for (const pk of hop.packages) {
      ...
      try {
        ...
        if (retryable) {
          ...
          if (!alive()) { try { await reconnect('při nahrávání balíčku'); } catch (e2) { L('warn', `připojení se nepovedlo (${e2.message}), zkusím to za chvíli`); await sleep(15000); try { await reconnect('opakovaně'); } catch (e3) { return { ok: false, error: `nahrání ${pk.file} selhalo a zařízení se neozývá: ${e3.message}`, client: c }; } } }
        ...
        if (!alive() && connect) { try { await reconnect('před fetch'); } catch (e2) { return { ok: false, error: `nahrání ${pk.file} selhalo: ${e.message}; zařízení se neozývá: ${e2.message}`, client: c }; } }
        const tk = token();
  ...
  async postBootCheck(c, before, after, L, W) {
    ...
    try {
      ...
      if (nb >= 2 && na < Math.ceil(nb / 2)) W(`po upgradu vidí zařízení jen ${na} sousedů (před ${nb}) — možná rozpadlý bridge nebo VLAN`);
  ...
  poeParentChecks(devId, info, settings) {
    ...
    const poeOf = (dev, n) => { const sc = dev && dev.flags && Array.isArray(dev.flags.poe_ports) ? dev.flags.poe_ports.length > 0 : null; return sc !== null ? sc : (POE_BOARD_RE.test((n && n.board) || '') || /poe/i.test((n && n.identity) || '')); };
    ...
    if (!powered) {
      for (const n of cands) {
        ...
        if (n.lldp === true || (me && d.id === me.parent_id)) { powered = d; break; }
      }
    }
    const seen = new Set();
    for (const n of cands) {
      ...
      if (!isPoe) continue;
      const direct = n.lldp === true; // nalezen přes LLDP → jeden kabel daleko
      ...
      if (dev && dev.managed && dev.enabled) {
        ...
        out.notes.push(`nad zařízením je PoE prvek ${devLabel(dev)} (${n.board || '?'}${direct ? ', přímo na kabelu' : farAway ? ', ale je dál v síti — nenapájí ho' : ''}${via}), je v seznamu — PoE watchdog: ${wds.length ? `${wds.map(w => `${w.kind} ${w.name}${w.iface ? ` (${w.iface})` : ''}`).join(', ')} → před restartem se vypne` : 'žádný (ověřeno ze skenu)'}`);
        continue;
      }
      if (farAway) {
      ...
      const msg = `nad zařízením je podle sousedů PoE prvek „${n.identity || '?'}" (${n.board || '?'}, ${n.address || n.mac || '?'}${direct ? `, přímo na kabelu${via}` : ''})${dev ? ' bez loginu' : ', který není v seznamu'} — neovladatelný PoE switch: nejde ověřit ani vypnout jeho PoE watchdog, není jistota, že potomkovi během restartu neodpojí napájení. Přidej ho skenem (s loginem), nebo upgraduj ručně.`;
  ...
  async doFirmware(c, connect, info, L, setStep, W, creds, settings, devId) {
    ...
    if (autoDone) L('info', `router má auto-upgrade firmware: nový RouterBOOT ${rb['upgrade-firmware']} už zapsal sám při startu (${fwLogBefore.lastOk.slice(0, 80)}) — jen restart`);
      ...
      if (fw !== 'ok') { W('zařízení do 2 min nepotvrdilo v logu zápis firmware — pro jistotu nerestartuji; firmware se aktivuje při příštím restartu, zkontroluj /log'); return c; }
    }
  ...
  async waitCycle(creds, connect, L, setStep, settings, downTimeoutMs) {
    ...
    L('error', `zařízení se nevrátilo do ${settings.reboot_timeout_min} min!`);
