...
class RunnerPool extends EventEmitter {
  ...
  for(ownerId) {
    ...
    if (!r) { r = new Runner(); r.ownerId = key; r.pool = this; r.on('event', (ev) => this.emit('event', ev)); this.runners.set(key, r); }
    return r;
  }
  ...
  tickScheduled() {
    ...
    for (const j of db.listJobs(200)) {
      if (j.status !== 'scheduled' || !(j.options && j.options.start_at) || j.options.start_at > now) continue;
      const r = this.for(j.owner_id);
      if (r.busy) continue; // uživatel má rozběhnutý jiný job → počká na další tik
      try { db.addLog(j.id, 0, 0, 'info', `Naplánované spuštění (${new Date(j.options.start_at * 1000).toLocaleString('cs-CZ')}).`); r.start(j.id); }
      catch (e) { db.addLog(j.id, 0, 0, 'error', 'naplánované spuštění selhalo: ' + e.message); }
    }
  }
}
