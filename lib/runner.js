  ...
  async precheckAll(jobId, opt, { early = false } = {}) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        if (raw.skip_link_check) p.notes.push('ověření bezdrátových spojů po restartu je u tohoto zařízení vypnuté — nečeká se na klienty, registraci k AP ani 60 GHz protějšek');
