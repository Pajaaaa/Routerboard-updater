...
class Runner extends EventEmitter {
  ...
  poeParentChecks(devId, info, settings) {
    ...
    for (const n of cands) {
      ...
      if (dev && dev.managed && dev.enabled) { out.notes.push(`nad zařízením je PoE prvek ${dev.name || dev.identity || dev.host} (${n.board || '?'}) — je v seznamu, případný PoE watchdog se před upgradem vypne`); continue; }
