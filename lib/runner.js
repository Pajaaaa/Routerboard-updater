...
class Runner extends EventEmitter {
  ...
  async precheckAll(jobId, opt, { early = false } = {}) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        { const on = Runner.overloadNote(c, info); if (on) L('info', `kontrola: ${info.identity}${on}`); }
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 16; iter++) {
        ...
        L('info', `${info.identity} · ${info.board_name || info.model} · ${info.arch} · RouterOS ${info.version} · fw ${info.fw_current}/${info.fw_upgrade} · flash ${(info.free_hdd / MB).toFixed(1)}/${(info.total_hdd / MB).toFixed(0)} MB · RAM ${(info.free_mem / MB).toFixed(0)} MB volné · uptime ${info.uptime}${Runner.overloadNote(c, info)}`);
        ...
        if (p.lowFreeMem && !lowRamRebooted && !rebootedByUs && !opt.dry_run && p.blockers.every(x => x.startsWith('málo volné RAM'))) {
          ...
          setStep('vypnutí nepoužívaných balíčků', 'checking');
          await this.disableUnusedPackages(c, info, L, W, settings, false); // vypnutí platí po restartu, který následuje
        ...
        if (p.lowFreeMem && opt.dry_run && !lowRamRebooted && p.blockers.every(x => x.startsWith('málo volné RAM'))) {
          lowRamRebooted = true; // dry run: jen ukázat, co by se před restartem kvůli RAM vypnulo
          await this.disableUnusedPackages(c, info, L, W, settings, true);
        }
        if (p.blockers.length) {
          if (hopsDone && p.nothingToDo) {
            // cíl dosažen i firmware, „blokátor“ (typicky málo volné RAM na 32 MB kusu) už ničemu nebrání → jen poznámka, ne varování
            for (const b of p.blockers) L('info', 'pozn. (už není co dělat): ' + b);
            L('info', `♥ Hotovo a díky za trpělivost: ${info.identity || raw.host} běží na RouterOS ${p.target} s aktuálním firmwarem, spoje i konfigurace ověřené. Ať dlouho slouží!`);
            return finish('done', { result: { hops: hopsDone } });
          }
          if (hopsDone) {
  ...
  /** Přetížené zařízení (CPU ≥ 90 %): limity příkazů rovnou ×3, ať se sběr stavu nerozpadne na timeoutech (RB711 32 MB na v7: každý get 3–12 s). Vrací text do logu. */
  static overloadNote(c, info) {
    if (!info || !(info.cpu_load >= 90)) return '';
    if (c && typeof c.markSlow === 'function' && c.slowFactor < 3) c.markSlow(`CPU ${info.cpu_load} %`, 3);
    return ` · CPU ${info.cpu_load} % — přetížené, limity příkazů ×${(c && c.slowFactor) || 3}`;
  }
  ...
  async disableBridgeStp(c, L, W, settings, dryRun) {
    ...
    if (!todo.length) { if (!rows.length) L('info', 'bridge STP: zařízení nemá bridge — nic k vypnutí'); else L('info', `bridge STP: ${rows.length === 1 ? 'bridge ' + rows[0].name + ' už má' : rows.length + ' bridgů už má'} protocol-mode ${[...new Set(rows.map(r => r['protocol-mode'] || '?'))].join('/')} — nic k vypnutí`); return; }
  ...
  /**
   * Balíčky, které se na kusu s málo RAM smí vypnout, když je prokazatelně nic nepoužívá: název → sondy [výraz RouterOS,
   * očekávaná hodnota, co sonda hlídá]. Sonda s jinou hodnotou nebo chybou (menu neexistuje, jiná verze) = balíček se nechává.
   * system, wireless, dhcp, security, advanced-tools a cokoli mimo seznam se nikdy nevypíná. 17.9.2026 RB711 10.107.124.26:
   * 32 MB RAM, 6.43 se všemi 18 balíčky (user-manager, hotspot, mpls, tr069…) = 6,3 MB volných a CPU 100 % — ostatních 80 kusů
   * s 32 MB má zapnutých 10–11 balíčků a limit splňují.
   */
  static get UNUSED_PKG_PROBES() {
    return {
      'user-manager': [['[:len [/tool user-manager user find]]', '0', 'uživatelé User Manageru'], ['[:len [/tool user-manager router find]]', '0', 'routery User Manageru']],
      hotspot: [['[:len [/ip hotspot find]]', '0', 'hotspot servery']],
      mpls: [['[/mpls ldp get enabled]', 'false', 'LDP'], ['[:len [/interface vpls find]]', '0', 'VPLS rozhraní'], ['[:len [/interface traffic-eng find]]', '0', 'TE tunely']],
      'tr069-client': [['[/tr069-client get enabled]', 'false', 'TR-069 klient']],
      openflow: [['[:len [/openflow find]]', '0', 'OpenFlow přepínače']],
      calea: [], // odposlech CALEA: menu /calea na 6.43 ani neexistuje, v síti se nepoužívá — bez sondy
      lcd: [], // jen podle desky (LCD má RB2011UiAS a CCR) — viz unusedPkgPlan
      ups: [['[:len [/system ups find]]', '0', 'UPS']],
      ntp: [['[/system ntp server get enabled]', 'false', 'NTP server (klient je v system)']],
      multicast: [['[:len [/routing pim interface find]]', '0', 'PIM'], ['[:len [/routing igmp-proxy interface find]]', '0', 'IGMP proxy']],
      ppp: [['[:len [/ppp secret find]]', '0', 'PPP secrets'], ['[:len [/interface find type~"^(ppp|pptp|l2tp|sstp|ovpn)"]]', '0', 'PPP/tunelová rozhraní'], ['[:len [/interface pppoe-server server find]]', '0', 'PPPoE server'], ['[/interface pptp-server server get enabled]', 'false', 'PPTP server'], ['[/interface l2tp-server server get enabled]', 'false', 'L2TP server'], ['[/interface sstp-server server get enabled]', 'false', 'SSTP server'], ['[/interface ovpn-server server get enabled]', 'false', 'OVPN server']],
      routing: [['[:len [/routing ospf network find]]', '0', 'OSPF'], ['[:len [/routing bgp peer find]]', '0', 'BGP'], ['[:len [/routing rip interface find]]', '0', 'RIP'], ['[:len [/routing filter find]]', '0', 'routing filtry'], ['[:len [/routing bfd neighbor find]]', '0', 'BFD sousedé']],
      ipv6: [['[:len [/ipv6 address find dynamic=no]]', '0', 'IPv6 adresy'], ['[:len [/ipv6 dhcp-client find]]', '0', 'DHCPv6 klient'], ['[:len [/ipv6 pool find]]', '0', 'IPv6 pooly']],
    };
  }
  /** Skript pro jedno exec: každá sonda vypíše „balíček#i=hodnota“, chyba = %%ERR%% (menu chybí, jiná verze). */
  static pkgProbeScript(names) {
    const P = Runner.UNUSED_PKG_PROBES;
    return names.flatMap(n => (P[n] || []).map(([expr], i) => `:do {:put ("${n}#${i}=" . [:tostr ${expr}])} on-error={:put "${n}#${i}=%%ERR%%"}`)).join('; ');
  }
  static parsePkgProbe(text) {
    const out = {};
    for (const line of String(text || '').replace(/\r/g, '').split('\n')) { const m = line.match(/^([\w-]+#\d+)=(.*)$/); if (m) out[m[1]] = m[2].trim().replace(/^no$/, 'false').replace(/^yes$/, 'true'); } // get enabled vrací na v6 i yes/no (tr069-client)
    return out;
  }
  /** Rozhodnutí podle sond: {disable: [{name, why}], keep: [{name, why}]} — jen pro zapnuté balíčky ze seznamu. */
  static unusedPkgPlan(enabledNames, boardName, values) {
    const P = Runner.UNUSED_PKG_PROBES;
    const disable = [], keep = [];
    for (const n of enabledNames) {
      if (!(n in P)) continue;
      if (n === 'lcd') { if (/UiAS|CCR|LCD/i.test(boardName || '')) keep.push({ name: n, why: `deska ${boardName} má LCD` }); else disable.push({ name: n, why: 'deska nemá LCD' }); continue; }
      if (n === 'calea') { disable.push({ name: n, why: 'odposlech CALEA se nepoužívá' }); continue; }
      const probes = P[n];
      const bad = probes.map(([, want, what], i) => ({ v: values[`${n}#${i}`], want, what })).filter(x => x.v === undefined || x.v === '%%ERR%%' || x.v !== x.want);
      if (!bad.length) disable.push({ name: n, why: probes.map(p => p[2]).join(', ') + ' — nic' });
      else keep.push({ name: n, why: bad.map(x => x.v === undefined || x.v === '%%ERR%%' ? `${x.what}: nejde ověřit` : `${x.what}: ${x.v}`).join(', ') });
    }
    return { disable, keep };
  }
  /**
   * Před restartem kvůli málo RAM vypnout balíčky, které nic nepoužívá (nastavení low_ram_disable_pkgs). Vypnutí platí až po
   * restartu — ten hned následuje. Vrací {disabled: [názvy]}.
   */
  async disableUnusedPackages(c, info, L, W, settings, dryRun) {
    const none = { disabled: [] };
    const enabled = (info.packages || []).filter(p => !p.disabled).map(p => p.name);
    const cand = enabled.filter(n => n in Runner.UNUSED_PKG_PROBES);
    if (!settings.low_ram_disable_pkgs) {
      if (cand.length) L('info', `balíčky: zapnuto ${enabled.length}, z toho by šlo (pokud se nepoužívají) vypnout a uvolnit RAM: ${cand.join(', ')} — krok „vypnout nepoužívané balíčky“ je v nastavení vypnutý`);
      return none;
    }
    if (!cand.length) { L('info', `balíčky: zapnuto jen ${enabled.join(', ')} — žádný vypnutelný, RAM tudy neuvolním`); return none; }
    let text;
    try { text = await c.exec(Runner.pkgProbeScript(cand), { timeoutMs: 90000 }); } catch (e) { W(`balíčky: sondy použití selhaly (${e.message}) — nic nevypínám`); return none; }
    const plan = Runner.unusedPkgPlan(cand, info.board_name, Runner.parsePkgProbe(text));
    for (const k of plan.keep) L('info', `balíček ${k.name} se nechává: ${k.why}`);
    if (!plan.disable.length) { L('info', 'balíčky: všechny zapnuté se používají nebo to nejde ověřit — nic nevypínám'); return none; }
    const names = plan.disable.map(d => d.name);
    if (dryRun) { L('info', `DRY RUN balíčky — před restartem kvůli RAM by se vypnuly: ${plan.disable.map(d => `${d.name} (${d.why})`).join('; ')}`); return none; }
    const out = await c.exec(`/system package disable [find name~"^(${names.join('|')})\\$"]`, { timeoutMs: 60000, allowError: true });
    if (/failure|error|invalid|no such|syntax|expected/i.test(out)) { W(`balíčky: vypnutí selhalo → ${out.trim().split('\n')[0].slice(0, 120)}`); return none; }
    const stay = enabled.filter(n => !names.includes(n));
    L('info', `balíčky vypnuty (platí po restartu): ${plan.disable.map(d => `${d.name} (${d.why})`).join('; ')} — zůstává ${stay.length}: ${stay.join(', ')}`);
