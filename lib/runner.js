    ...
    for (const s of st) {
      ...
      for (const cand of cands.slice(0, 6)) {
        try {
          ...
          if (!cr) { L && L('info', `sektor? ${cand.identity || cand.host} (${cand.host}, ${cand.why}): userdb nemá login`); continue; }
          ...
          try {
            ...
            if (!live) continue; // není to on (rozhraní s MAC rádia tam není)
        ...
        } catch (e) { L && L('info', `sektor? ${cand.identity || cand.host} (${cand.host}): nepřipojil se přes userdb (${String(e.message).slice(0, 60)})`); }
      }
      if (!found) L && L('info', `sektor ${s.ap.mac} není v seznamu zařízení a mezi ${cands.length} kandidáty (sousedé, userdb) se nenašel — country/frequency-mode se neověří`);
    }
  }
