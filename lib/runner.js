...
function physicallyAdjacent(a, b) {
  ...
  const ips = (d, f) => [d.host, ...(Array.isArray(f.ip_addresses) ? f.ip_addresses : [])].filter(Boolean);
  const byAddr = (n, d, f) => !!(n && n.address) && ips(d, f).includes(n.address);
...
class RunnerPool extends EventEmitter {
  constructor() {
    super();
    this.draining = false; // před aktualizací serveru (deploy): joby dokončí aktuální zařízení a pozastaví se
  ...
  conflictFor(raw, self) {
    for (const r of this.runners.values()) {
      ...
      if (other && physicallyAdjacent(raw, other)) { const u = db.getUser(r.ownerId); return { name: other.name || other.identity || other.host, host: other.host, user: u ? (u.userdb_nick || u.name) : '?', jobId: r.currentJobId }; }
    }
    return null;
  }
  ...
  setDraining(on) { this.draining = !!on; if (!on) this.tickScheduled(); return this.draining; }
  tickScheduled() {
