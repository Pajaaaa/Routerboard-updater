...
class Runner extends EventEmitter {
  ...
  async runJob(jobId) {
    ...
    if (opt.precheck && !opt.precheckDone && !opt.dry_run && !opt.op) {
      ...
      if (res.blocked || res.warned) {
        const note = `kontrola hotová: ${res.ready} připraveno, ${res.blocked} se přeskočí${res.warned ? `, ${res.warned} s upozorněním` : ''} — „Pokračovat" spustí upgrade`;
        db.updateJob(jobId, { status: 'waiting', status_note: note });
        db.addLog(jobId, 0, 0, 'info', 'Kontrola hotová, čekám na potvrzení: ' + note);
        this.emitJob(jobId);
        return;
      }
      db.addLog(jobId, 0, 0, 'info', `Kontrola hotová: všech ${res.ready} zařízení připraveno, pokračuji upgradem.`);
   ...
   * Idempotentní — přidá jen, co chybí (akce s jiným cílem se přenastaví, vypnuté pravidlo se zapne). Dry run jen vypíše.
   */
  async setupRemoteLogging(c, L, W, settings, dryRun) {
    ...
    const name = String(settings.remote_log_name || '').trim();
    if (!host || !name || !/^[\w.-]+$/.test(name)) { W('vzdálené logování: v nastavení chybí (nebo je neplatná) adresa serveru / název akce — přeskakuji'); return; }
    ...
    if (!a) changes.push({ cmd: `/system logging action add name="${name}" target=remote remote=${host}`, desc: `akce ${name} → ${host} (nová)` });
    else {
      let remote = '';
      try { remote = (await c.exec(`:put [/system logging action get [find name="${name}"] remote]`, { timeoutMs: 10000 })).trim(); } catch {}
      if (a.target !== 'remote' || remote !== host) changes.push({ cmd: `/system logging action set [find name="${name}"] target=remote remote=${host}`, desc: `akce ${name}: target=remote remote=${host} (bylo target=${a.target} remote=${remote || '?'})` });
    }
