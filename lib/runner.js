...
class Runner extends EventEmitter {
  ...
  async precheckAll(jobId, opt, { early = false } = {}) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, allow_v7: !!raw.allow_v7, ignore_flagged: !!raw.ignore_flagged, skip_link_check: !!raw.skip_link_check, prev_bad_blocks: pf.bad_blocks, job_client_macs: jobMacs.filter(m => m.device_id !== it.device_id) } });
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 16; iter++) {
        ...
        if (!raw.skip_link_check) await this.enrichStationAps(info, L, raw);
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, ignore_uptime: rebootedByUs, allow_v7: !!raw.allow_v7, ignore_flagged: !!raw.ignore_flagged, skip_link_check: !!raw.skip_link_check, prev_bad_blocks: prevFlags.bad_blocks } });
  ...
  async verifyLinks(c, before, after, L, setStep, W, settings) {
    ...
    const volatile = (cl) => { const b = parseInt(String(cl.mac || '').slice(0, 2), 16); return Number.isFinite(b) && (b & 2) === 2 && !cl.version; };
    ...
    while (true) {
      ...
      for (const a of aps) {
        ...
        if (vol && !volatileNoted.has(a.iface)) { volatileNoted.add(a.iface); L('info', `sektor ${a.iface}: ${vol} z ${a.clients.length} klientů má náhodnou MAC (telefon/notebook) — do návratu se nepočítají${stable.length ? '' : ', návrat klientů se u tohoto sektoru nevyhodnocuje'}`); }
        if (!stable.length) { summary.push(`${a.iface}: jen ${vol} klientů s náhodnou MAC, nehodnotí se`); continue; }
  ...
  poeParentChecks(devId, info, settings) {
    ...
    for (const n of cands) {
      ...
      const msg = `nad zařízením je podle sousedů PoE prvek „${n.identity || '?'}" (${n.board || '?'}, ${n.address || n.mac || '?'}${dist})${dev ? ' bez loginu' : ', který není v seznamu'} — neovladatelný PoE switch: nejde ověřit ani vypnout jeho PoE watchdog, není jistota, že potomkovi během restartu neodpojí napájení. Přidej ho skenem (s loginem), nebo upgraduj ručně.`;
