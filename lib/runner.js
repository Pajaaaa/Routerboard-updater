  ...
  async verifyLinks(c, before, after, L, setStep, W, settings) {
    ...
    while (true) {
      ...
      for (const a of aps) {
        ...
        else summary.push(`${a.iface}: ${back.length}/${a.clients.length} klientů zpět${roamNote}`);
      }
      for (const w of w60) {
  ...
  async clientsOnSiblingSectors(ssid, macs, exceptId) {
    ...
    for (const d of sib) {
      ...
      try {
        ...
        for (const line of String(txt).split(/\r?\n/)) { const m = line.trim().toUpperCase(); if (want.has(m)) out.set(m, d.name || d.identity || d.host); }
