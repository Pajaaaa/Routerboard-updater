  ...
  async precheckAll(jobId, opt) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, allow_v7: !!raw.allow_v7 } });
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 6; iter++) {
        ...
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, ignore_uptime: rebootedByUs, allow_v7: !!raw.allow_v7 } });
        ...
        if (p.lowFreeMem && !lowRamRebooted && !rebootedByUs && !opt.dry_run && p.blockers.length === 1) {
          lowRamRebooted = true;
...
class RunnerPool extends EventEmitter {
  ...
  for(ownerId) {
    ...
    if (!r) { r = new Runner(); r.ownerId = key; r.on('event', (ev) => this.emit('event', ev)); this.runners.set(key, r); }
    return r;
  }
...
module.exports = { Runner, RunnerPool, inWindow };
