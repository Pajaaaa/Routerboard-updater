...
const POE_BOARD_RE = /PoE|netPower|PowerBox|OmniTIK|CRS[13]\d\d-\d+P|PGS|RB750UP|hAP ac\^?[23]|hAP ax|wAP ac|wAP ax|cAP ac|cAP ax|RB4011|RB3011|RB5009|RB2011UiAS|RB1100AHx4|RB960/i; // min bez aktivity položky, po kterých watchdog přeruší visící SSH spojení
...
class Runner extends EventEmitter {
  ...
  async precheckAll(jobId, opt) {
    ...
    for (const it of db.getJobItems(jobId)) {
      ...
      try {
        ...
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, allow_v7: !!raw.allow_v7, prev_bad_blocks: pf.bad_blocks } });
  ...
  async runItem(job, item, opt) {
    ...
    try {
      ...
      for (let iter = 0; iter < 6; iter++) {
        ...
        const p = await plan(info, { track: raw.track, settings, latest: V.getLatest(), options: { ...opt, ignore_uptime: rebootedByUs, allow_v7: !!raw.allow_v7, prev_bad_blocks: prevFlags.bad_blocks } });
      ...
      this.currentClient = null; this.currentRaw = null;
    }
  }
  ...
  async pauseParentPoeWatchdogs(raw, L, W, settings) {
    ...
    const up = fr.uplink;
    const viaNeighbor = up ? this.deviceOfNeighbor(up.neighbor || (up.neighbors_on_iface || [])[0], raw.id) : null;
    ...
    const label = par.name || par.identity || par.host;
    ...
    while (true) {
      ...
      for (const w of w60) {
        ...
        if (/^station/.test(w.mode || '') ? missing.length : (back.length * 100 / Math.max(1, w.stations.length)) < pct) problems.push(`60 GHz ${w.iface}: chybí protějšek ${missing.map(x => x.mac).join(', ')} (zpět ${back.length}/${w.stations.length})`);
          ...
          if (cur.mcs !== null && w.mcs !== null && cur.mcs < w.mcs - 2) W(`60 GHz ${w.iface}: MCS po restartu klesl ${w.mcs} → ${cur.mcs}`);
          for (const x of cur.stations || []) { if (x.signal !== null && x.signal < Number(settings.radio_min_signal60 ?? 50)) W(`60 GHz ${w.iface} → ${x.mac}: slabá kvalita po restartu ${x.signal} %`); if (x.per !== null && x.per > Number(settings.radio_max_per ?? 5)) W(`60 GHz ${w.iface} → ${x.mac}: chybovost po restartu ${x.per} %`); }
        }
      }
      if (bl.cap) {
  ...
  deviceOfNeighbor(n, exceptId) {
    if (!n) return null;
    return db.listDevices().find(d => d.id !== exceptId && ((n.address && d.host === n.address) || (n.identity && d.identity && d.identity === n.identity))) || null;
  }
  ...
  poeParentChecks(devId, info, settings) {
    ...
    for (const n of cands) {
      ...
      const isPoe = POE_BOARD_RE.test(n.board || '') || /poe/i.test(n.identity || '');
      if (!isPoe) continue;
      const dev = this.deviceOfNeighbor(n, devId);
      if (dev && dev.managed && dev.enabled) {
        const wds = ((dev.flags && dev.flags.poe_watchdogs) || []).filter(w => !w.disabled);
        out.notes.push(`nad zařízením je PoE prvek ${dev.name || dev.identity || dev.host} (${n.board || '?'}), je v seznamu — PoE watchdog: ${wds.length ? `${wds.map(w => `${w.kind} ${w.name}${w.iface ? ` (${w.iface})` : ''}`).join(', ')} → před restartem se vypne` : 'žádný (ověřeno ze skenu)'}`);
        continue;
      }
  ...
  async doFirmware(c, connect, info, L, setStep, W, creds, settings, devId) {
    ...
    L('info', 'příkaz upgrade RouterBOOT odeslán' + (out.trim() ? ': ' + out.trim().split('\n')[0] : ' (router nic nevypsal, to je běžné — ověří se po restartu)'));
...
function physicallyAdjacent(a, b) {
  ...
  const match = (n, d, f) => !!n && ((n.address && n.address === d.host) || (n.identity && d.identity && n.identity === d.identity) || (n.mac && radioMacs(f).includes(up(n.mac))));
  const nbs = (f) => [f.uplink && f.uplink.neighbor, ...((f.uplink && f.uplink.neighbors_on_iface) || []), ...(f.poe_children || [])].filter(Boolean);
  if (nbs(fa).some(n => match(n, b, fb)) || nbs(fb).some(n => match(n, a, fa))) return true;
  const peers = (f) => { const l = f.links || {}; return [...(l.stations || []).map(s => s.ap && s.ap.mac), ...(l.w60g || []).flatMap(w => (w.stations || []).map(x => x.mac)), ...(l.aps || []).flatMap(x => (x.clients || []).map(cl => cl.mac))].map(up).filter(Boolean); };
...
class RunnerPool extends EventEmitter {
  constructor() {
    super();
    this.runners = new Map(); // ownerId -> Runner
  ...
  conflictFor(raw, self) {
    for (const r of this.runners.values()) {
      ...
      if (other && physicallyAdjacent(raw, other)) { const u = db.getUser(r.ownerId); return { name: other.name || other.identity || other.host, host: other.host, user: u ? u.name : '?' }; }
    }
    return null;
  }
  runnerOfJob(jobId) { for (const r of this.runners.values()) if (r.currentJobId === jobId) return r; return null; }
  ...
  status(ownerId) { const r = this.runners.get(Number(ownerId) || 0); return r ? r.status() : { running: false, ownerId: Number(ownerId) || 0, jobId: 0, itemId: 0, deviceId: 0 }; }
  /** všechny právě běžící joby (pro správce) */
  running() { return [...this.runners.values()].filter(r => r.busy).map(r => r.status()); }
  start(jobId) {
    const job = db.getJob(jobId);
    if (!job) throw new Error('job neexistuje');
    return this.for(job.owner_id).start(jobId);
  }
  /** spustit hned, nebo — když uživateli něco běží — zařadit do fronty (spustí se samo po dokončení); vrací {queued, behind} */
  startOrQueue(jobId) {
    const job = db.getJob(jobId);
    if (!job) throw new Error('job neexistuje');
    if (['done', 'cancelled'].includes(job.status)) throw new Error('job je už ukončený');
    const r = this.for(job.owner_id);
    if (!r.busy) { r.start(jobId); return { queued: false }; }
    const behind = r.currentJobId;
    db.updateJob(jobId, { status: 'queued', status_note: `ve frontě — spustí se po dokončení jobu #${behind}`, options: { ...job.options, queued_at: Date.now() / 1000 } });
    db.addLog(jobId, 0, 0, 'info', `Zařazeno do fronty za job #${behind}; spustí se samo, až doběhne.`);
    return { queued: true, behind };
  }
  repartition(devId, count) { const d = db.getDevice(devId); if (!d) throw new Error('zařízení neexistuje'); return this.for(d.owner_id).repartition(devId, count); }
  getPkg(tk) { for (const r of this.runners.values()) { const p = r.getPkg(tk); if (p) return p; } return null; }
  emitJob(jobId) { this.emit('event', { type: 'job', job: db.listJobs(200).find(j => j.id === jobId) || null }); }
  emitItem(itemId) { const it = db.getJobItem(itemId); if (it) this.emit('event', { type: 'item', item: it }); }
  tickScheduled() {
    const now = Date.now() / 1000;
    const jobs = db.listJobs(200);
    // naplánované (čas nastal) i zařazené do fronty; v pořadí zařazení, jeden na uživatele
    const ready = jobs.filter(j => (j.status === 'scheduled' && j.options && j.options.start_at && j.options.start_at <= now) || (j.status === 'queued' && j.options && j.options.queued_at))
      .sort((a, b) => ((a.options.queued_at || a.options.start_at) - (b.options.queued_at || b.options.start_at)));
    for (const j of ready) {
      const r = this.for(j.owner_id);
      if (r.busy) continue; // uživatel má rozběhnutý jiný job → počká na další tik
      try {
        db.addLog(j.id, 0, 0, 'info', j.status === 'scheduled' ? `Naplánované spuštění (${new Date(j.options.start_at * 1000).toLocaleString('cs-CZ')}).` : 'Spuštění z fronty (předchozí job doběhl).');
        r.start(j.id);
      } catch (e) { db.addLog(j.id, 0, 0, 'error', 'spuštění z fronty selhalo: ' + e.message); }
    }
  }
  /** po skončení jobu hned zkusit frontu (nečekat na tik) */
  for(ownerId) {
    const key = Number(ownerId) || 0;
    let r = this.runners.get(key);
    if (!r) { r = new Runner(); r.ownerId = key; r.pool = this; r.on('event', (ev) => { this.emit('event', ev); if (ev.type === 'runner' && ev.status && !ev.status.running) setTimeout(() => this.tickScheduled(), 1500); }); this.runners.set(key, r); }
    return r;
  }
}
