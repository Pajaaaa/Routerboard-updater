...
for (const [tbl, col, ddl] of [['devices', 'owner_id', 'INTEGER DEFAULT 0'], ['jobs', 'owner_id', 'INTEGER DEFAULT 0'], ['devices', 'allow_v7', 'INTEGER DEFAULT 0']]) {
...
const DEVICE_COLS = ['id','host','port','username','name','group_name','priority','enabled','track','notes','parent_id','managed',
  ...
  'scan_status','scan_error','last_scan_at','last_seen_at','last_upgrade_at','created_at','updated_at','host_key','owner_id','allow_v7'];
...
function updateDevice(id, fields) {
  const allowed = ['host','port','username','password_enc','name','group_name','priority','enabled','track','notes','host_key','parent_id','managed','owner_id','allow_v7',
