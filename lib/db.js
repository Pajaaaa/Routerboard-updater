...
function updateDevice(id, fields) {
  const allowed = ['host','port','username','password_enc','name','group_name','priority','enabled','track','notes','host_key','parent_id','managed','owner_id','allow_v7','ignore_poe',
...
// ---- uživatelé ----
const userPub = (u) => u ? { id: u.id, name: u.name, role: u.role, disabled: !!u.disabled, created_at: u.created_at, last_login_at: u.last_login_at, has_password: !!u.pass_hash } : null;
...
function updateUser(id, f) {
  const sets = [], vals = [];
  for (const k of ['name', 'pass_hash', 'role', 'disabled', 'last_login_at']) if (k in f) { sets.push(`${k}=?`); vals.push(typeof f[k] === 'boolean' ? (f[k] ? 1 : 0) : f[k]); }
  if (!sets.length) return;
