...
if (!devCols.has('parent_src')) db.exec("ALTER TABLE devices ADD COLUMN parent_src TEXT DEFAULT ''"); // jak byl rodič určen: manual | radio | capsman | gateway ('' = neznámo/starší záznam)
...
// ---- devices ----
const DEVICE_COLS = ['id','host','port','username','name','group_name','priority','enabled','track','notes','parent_id','parent_src','managed','userdb_ap_id','userdb_ap','userdb_member',
...
function updateDevice(id, fields) {
  const allowed = ['host','port','username','password_enc','name','group_name','priority','enabled','track','notes','host_key','parent_id','parent_src','managed','owner_id','allow_v7','ignore_poe','userdb_ap_id','userdb_ap','userdb_member',
...
module.exports = { getUserByUserdbUid,
