...
async function api(req, res, method, p, url) {
  ...
  if (seg[0] === 'devices' && seg[1]) {
    ...
    if (method === 'PUT' && !seg[2]) {
      const b = await readBody(req);
      const f = {};
      for (const k of ['host', 'port', 'username', 'name', 'group_name', 'priority', 'enabled', 'track', 'notes', 'parent_id', 'managed', 'allow_v7', 'ignore_poe', 'ignore_flagged', 'skip_link_check']) if (k in b) f[k] = b[k];
