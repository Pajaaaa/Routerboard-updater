...
function renderJobs(m) {
  ...
  const owners = state.admin ? [...new Map(state.jobs.filter(j => j.owner_id).map(j => [j.owner_id, j.owner_name || ('#' + j.owner_id)])).entries()].sort((x, y) => x[1].localeCompare(y[1], 'cs')) : [];
  ...
  const jo = $('#jobowner'); if (jo) jo.onchange = (e) => { state.jobOwner = +e.target.value || 0; render(); };
