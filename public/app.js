...
/** štítek „vlastní: X“ u zařízení jiného uživatele (správce vidí všechna zařízení; běžný uživatel jen svoje, takže se mu neukáže) */
const foreignChip = (d) => { const me = (state.auth.user || {}).id; if (!d.owner_id || !me || d.owner_id === me) return ''; const u = (state.users || []).find(x => x.id === d.owner_id); return ` <span class="chip chip-foreign" title="zařízení má u sebe jiný uživatel — upgrade mu spustí on; převzít jde přes Natáhnout z userdb → Převzít k sobě, nebo tužkou (vlastník)">👤 vlastní: ${esc(u ? (u.userdb_nick || u.name) : '#' + d.owner_id)}</span>`; };
...
function renderHelp(m) {
  ...
  <details><summary>„country × sektor“ / „frequency-mode × sektor“ — stanice se po upgradu nenaladí</summary><p>Skutečná příčina mrtvých antén po upgradu (9. 9. 2026): stanice měla jiné <b>country</b> nebo <b>frequency-mode</b> než sektor, po upgradu RouterOS regulaci vynutí a k sektoru naladěnému na frekvenci mimo „zemi“ stanice se už nepřipojí. Nástroj proto u každé stanice porovnává obě hodnoty se sektorem ze seznamu zařízení a při rozdílu položku blokuje; sjednoť je na stanici (Wireless → country, frequency-mode) a spusť znovu. Když sektor v seznamu není, je to jen varování — ověř ručně.</p></details>
