...
function renderDevices(m) {
  ...
  ${(state.renderedIds = new Set(list.map(d => d.id)), '')}${list.map(d => { const ps = plainStatus(d); const sc = STATUS_LABEL[d.scan_status] || ['b-muted', d.scan_status]; const scanning = state.scanning.includes(d.id); const busy = (state.runner.busy || []).includes(d.id);
    ...
    <td>${d._ctx ? '' : `<input type="checkbox" class="sel" data-id="${d.id}" ${state.selected.has(d.id) ? 'checked' : ''}>`}</td>
    ...
    <td class="mono">${d.dup_of ? '<span class="muted" title="stejný kus jako hlavní záznam — verze je u něj">viz hlavní</span>' : `<b>${esc(d.version || '—')}</b>${adv && d.channel ? ` <span class="muted">${esc(d.channel)}</span>` : ''}${adv && d.uptime_sec ? `<span class="sub" title="uptime">${upt(d.uptime_sec)}</span>` : ''}`}</td>
    <td>${scanning ? '<span class="badge b-info">kontroluji…</span>' : `<span class="badge ${ps.cls}" title="${esc(d.scan_error || '')}">${esc(ps.txt)}</span>`}</td>
    ${adv ? `<td class="mono">${d.dup_of ? '' : `${esc(d.fw_current || '—')}${d.fw_upgrade && d.fw_current !== d.fw_upgrade ? ` <span class="badge b-warn" title="k dispozici ${esc(d.fw_upgrade)}">→${esc(d.fw_upgrade)}</span>` : ''}`}</td>
