    ...
    if (udbOn) {
      ...
      const load = async () => {
        ...
        box.innerHTML = `<p>Správce <b>${esc(r.admin.nick)}</b> (uid ${r.admin.id}): ${r.areas.length} oblastí, ${nAps} APček.</p>
          <div class="tablewrap"><table><thead><tr><th><input type="checkbox" id="apall" checked title="vše / nic"></th><th>oblast</th><th>APčko</th><th>zařízení AP</th><th>zařízení členů</th><th>už v upgraderu</th></tr></thead><tbody>${r.areas.map(a => a.aps.map(ap => `<tr><td><input type="checkbox" class="apsel" value="${ap.id}" ${ap.active ? 'checked' : ''}></td><td>${esc(a.name)} <span class="muted">(${a.role === 'SO' ? 'správce' : 'zástupce'})</span></td><td>${esc(ap.name)}${ap.active ? '' : ' <span class="badge b-muted">neaktivní</span>'}</td><td>${ap.total - ap.members}</td><td>${ap.members}</td><td>${ap.imported ? `<b>${ap.imported}</b>` : '<span class="muted">0</span>'}</td></tr>`).join('')).join('')}</tbody></table></div>
