...
function renderSettings(m, adminMode = false) {
  ...
  const sm = $('#setf-mine'); if (sm) sm.onsubmit = async (e) => { e.preventDefault(); try { const r = await api('/settings/mine', { method: 'PUT', body: formBody(e.target) }); state.settings = r.settings; state.settingsOwn = r.own; toast('moje nastavení uloženo'); render(); } catch (e2) { toast(e2.message, true); } };
