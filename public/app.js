...
function renderHelp(m) {
  ...
  <details><summary>„v7 se na tenhle HW nenahrává“ / cíl přepnut na v6</summary><p>Pevný seznam v nástroji: MIPS-LE (RB100/RB500, v7 neexistuje), cokoli pod 64 MB RAM (MikroTik: „should not run v7“, hrozí OOM bootloop), smips (hAP lite/mini, cAP lite), staré RB411/433/450/493 a 32 MB kusy jako RB711, RB750, RB751, RB951-2n, SXT G, OmniTIK 5, Groove. Taková zařízení nástroj sám cílí na poslední v6 long-term. Kdo v7 chce i tak, zaškrtne u zařízení (tužka ✎) „povolit v7 i přes doporučení“; u MIPS-LE to nejde vůbec.</p></details>
...
function renderModal() {
  ...
  if (md.type === 'move') {
    ...
    $('#editf').onsubmit = async (e) => { e.preventDefault(); const b = Object.fromEntries(new FormData(e.target)); b.enabled = !!b.enabled; b.managed = !b.unmanaged; delete b.unmanaged; b.allow_v7 = !!b.allow_v7; b.parent_id = +b.parent_id; b.port = +b.port; b.priority = +b.priority; if ('owner_id' in b) b.owner_id = +b.owner_id; if (!b.password) delete b.password; try { await api(`/devices/${d.id}`, { method: 'PUT', body: b }); toast('uloženo'); closeModal(); await loadState(); render(); } catch (e2) { toast(e2.message, true); } };
