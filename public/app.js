...
function renderModal() {
  ...
  if (md.type === 'move') {
    ...
    bg.innerHTML = `<div class="modal"><h2>Upravit ${esc(devLabel(d))}</h2><form id="editf" class="form">
      ...
      ${(d.flags && d.flags.device_mode && d.flags.device_mode.flagged) || d.ignore_flagged ? `<label class="check"><input type="checkbox" name="ignore_flagged" ${d.ignore_flagged ? 'checked' : ''}> device-mode je flagged: zařízení jsem prověřil a uklidil, upgradovat i tak <span class="muted">(bez fyzického odblokování zůstane omezení scheduleru, VPN a proxy; upgrade tím není blokován)</span></label>` : ''}
