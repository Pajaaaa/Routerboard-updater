...
function renderJobs(m) {
  ...
  const owners = state.admin ? [...new Map([...(state.users || []).map(u => [u.id, u.userdb_nick || u.name]), ...state.jobs.filter(j => j.owner_id).map(j => [j.owner_id, j.owner_name || ('#' + j.owner_id)])]).entries()].filter(([id]) => jc[id]).sort((x, y) => x[1].localeCompare(y[1], 'cs')) : [];
