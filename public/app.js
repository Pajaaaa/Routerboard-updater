...
// zařízení čeká na upgrade v naplánovaném / zařazeném / běžícím jobu → malé hodiny u jména
const CLOCK_SVG = '<svg viewBox="0 0 16 16" width="12" height="12" aria-hidden="true"><circle cx="8" cy="8" r="6.4" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M8 4.3V8.2l2.6 1.6" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
function schedIcon(d) {
  const s = ((state.runner && state.runner.sched) || {})[d.id];
  if (!s) return '';
...
function render() {
  ...
  $('#refreshver').onclick = async () => { state.latest = { ...await api('/versions/refresh', { method: 'POST' }), floor: (state.latest || {}).floor }; toast('verze obnoveny'); render(); };
...
// a vezme se název z evidence / ručně zadaný, nakonec IP. Stejná logika jako lib/label.js na serveru.
const genericIdentity = (d) => { const id = String(d.identity || '').trim(); if (!id || /^mikrotik$/i.test(id)) return true; return [d.board_name, d.model].map(x => String(x || '').trim().toLowerCase()).filter(Boolean).includes(id.toLowerCase()); };
/** nenápadná poznámka „vlastní X“ u zařízení jiného uživatele (správce vidí všechna zařízení; běžný uživatel jen svoje, takže se mu neukáže) */
const foreignChip = (d) => { const me = (state.auth.user || {}).id; if (!d.owner_id || !me || d.owner_id === me) return ''; const u = (state.users || []).find(x => x.id === d.owner_id); return ` <span class="muted" title="zařízení má u sebe jiný uživatel — upgrade mu spustí on; převzít jde přes Natáhnout z userdb → Převzít k sobě, nebo tužkou (vlastník)">· vlastní ${esc(u ? (u.userdb_nick || u.name) : '#' + d.owner_id)}</span>`; };
const devLabel = (d) => { if (!d) return ''; const name = d.name !== undefined ? d.name : d.dev_name; return genericIdentity(d) ? (name || d.identity || d.host || '') : d.identity; };
const devName = devLabel;
...
function filteredDevices() {
  ...
  list.sort((a, b) => {
    ...
    else if (s === 'name') r = (devLabel(a)).localeCompare(devLabel(b));
...
function renderDevices(m) {
  ...
  ${(state.renderedIds = new Set(list.map(d => d.id)), '')}${list.map(d => { const ps = plainStatus(d); const sc = STATUS_LABEL[d.scan_status] || ['b-muted', d.scan_status]; const scanning = state.scanning.includes(d.id); const busy = (state.runner.busy || []).includes(d.id);
    ...
    <td data-l="stav">${scanning ? '<span class="badge b-info">kontroluji…</span>' : `<span class="badge ${ps.cls}" title="${esc(d.scan_error || '')}">${esc(ps.txt)}</span>`}</td>
    ${adv ? `<td class="mono" data-l="firmware">${d.dup_of ? '' : `${esc(d.fw_current || '—')}${d.fw_upgrade && d.fw_current !== d.fw_upgrade ? ` <span class="badge b-warn" title="k dispozici ${esc(d.fw_upgrade)}">→${esc(d.fw_upgrade)}</span>` : ''}`}</td>
    <td data-l="flash / RAM">${d.total_hdd ? `<span title="flash volné / celkem">${mb(d.free_hdd)}/${mb(d.total_hdd)}</span>${d.flags && d.flags.flash_dir ? '<span class="muted" title="kořen FS v RAM, adresář flash">ᶠ</span>' : ''} <span class="muted">·</span> <span title="RAM volná / celkem">${mb(d.free_mem)}/${mb(d.total_mem)}</span> <span class="muted">MB</span>` : '—'}</td>
    <td class="parent" data-l="nadřazený">${parentCell(d)}</td>
    <td data-l="kanál">${d.managed ? `<select class="track small" data-id="${d.id}">${state.tracks.map(t => `<option value="${t}" ${t === d.track ? 'selected' : ''}>${t.replace('long-term', 'LT')}</option>`).join('')}</select>` : '<span class="badge b-muted">neřízený</span>'}</td>
    <td data-l="sken">${!d.managed ? '<span class="muted">—</span>' : `<span class="badge ${sc[0]}" title="${esc(d.scan_error)}">${esc(sc[1])}</span> <span class="muted" title="naposledy viděno ${fmtTs(d.last_seen_at)}">${ago(d.last_seen_at)}</span>`}</td>${state.admin && state.users ? `<td class="muted" data-l="vlastník">${esc((state.users.find(u => u.id === d.owner_id) || {}).name || '—')}</td>` : ''}` : ''}
  ...
  ${list.length ? '' : `<tr><td colspan="${adv ? 11 : 6}" class="empty">Zatím žádná zařízení. Přidej je skenem: zadáš IP adresy nebo rozsahy a loginy, nalezená zařízení se založí sama.</td></tr>`}
  </tbody></table></div></div>`;
...
function renderJobs(m) {
  ...
  // správce vidí joby všech — filtr podle vlastníka (state.jobOwner = id účtu, 0 = všichni); seznam vlastníků z načtených jobů
  // seznam uživatelů pro filtr: všechny účty (state.users), ne jen vlastníci načtených jobů — po načtení historie jednoho uživatele by ostatní zmizeli
  ...
  // vždy jen to, co právě běží nebo čeká na „Pokračovat“; naplánované, ve frontě, pozastavené a hotové jen posledních N podle výběru (0 = vše)
  // (naplánovaných a ve frontě bývá i 15 — když se ukazovaly vždy, výběr počtu nebyl znát)
  const LIVE_JOB = ['running', 'waiting', 'waiting-window'];
  ...
  void ACTIVE_JOB;
  m.innerHTML = `<h1>Upgrady</h1><div class="stack"><div class="panel"><h2>Přehled <span class="muted" style="font-weight:400;font-size:12px">${live.length ? `${live.length} běžících + ` : ''}${lim ? `posledních ${Math.min(lim, rest.length)}` : `všech ${rest.length}`}</span>${owners.length ? ` <select id="jobowner" title="jen joby jednoho správce" style="margin-left:8px;font-weight:400;font-size:13px"><option value="0">všichni uživatelé</option>${owners.map(([id, nm]) => `<option value="${id}" ${state.jobOwner === id ? 'selected' : ''}>${esc(nm)}${state.jobCounts ? ` (${jc[id] || 0})` : ''}</option>`).join('')}</select>` : ''} <select id="joblimit" title="kolik jobů zobrazit kromě právě běžících (ty jsou vždy); od nejnovějšího, včetně naplánovaných a pozastavených" style="margin-left:8px;font-weight:400;font-size:13px">${[10, 20, 30, 50, 0].map(n => `<option value="${n}" ${lim === n ? 'selected' : ''}>${n ? `zobrazit ${n}` : 'zobrazit vše'}</option>`).join('')}</select></h2><div class="tablewrap"><table class="cards"><thead><tr>${adv ? '<th>#</th>' : ''}<th>Název</th><th>Stav</th><th>Průběh</th><th>Vytvořen</th><th></th></tr></thead><tbody>
    ${shown.map((j, i) => { const c = j.counts || {}; const done = (c.done || 0) + (c.failed || 0) + (c.blocked || 0) + (c.skipped || 0) + (c.unknown || 0);
      void i;
      return `<tr class="clickable ${cur && cur.job.id === j.id ? 'selected' : ''}" data-id="${j.id}">${adv ? `<td data-l="číslo">${j.id}</td>` : ''}<td class="cardhead">${esc(j.name)}${state.admin && j.owner_name && j.owner_id !== (state.auth.user || {}).id ? ` <span class="muted" title="kdo job spustil">· ${esc(j.owner_name)}</span>` : ''}${j.options.dry_run ? ' <span class="badge b-info">jen kontrola</span>' : ''}${j.options.op ? ' <span class="badge b-muted">operace</span>' : ''}</td><td data-l="stav">${jobBadge(j)}${j.current ? `<div class="muted" style="font-size:11px;white-space:normal">${esc(j.current.dev)}${j.current.step ? ` — ${esc(j.current.step)}` : ''}</div>` : ''}${(c.blocked || 0) && j.status !== 'done' ? `<div class="muted" style="font-size:11px">${c.blocked} přeskočeno (blokováno)</div>` : ''}${adv && j.status_note && !j.current ? `<div class="muted" style="font-size:11px;white-space:normal">${esc(j.status_note)}</div>` : ''}</td>
      <td data-l="průběh">${done}/${j.total} <span class="muted">(${c.done || 0} ok${c.failed ? `, <span style="color:var(--err)">${c.failed} chyb</span>` : ''}${c.blocked ? `, ${c.blocked} přeskočeno` : ''})</span><div class="progress"><div style="width:${j.total ? done / j.total * 100 : 0}%"></div></div></td><td data-l="vytvořen">${fmtTs(j.created_at)}</td>
      <td class="acts">${[['queued', 'paused'].includes(j.status) ? `<button class="small ok jstart" data-id="${j.id}">▶ Spustit</button>` : '', j.status === 'waiting' ? `<button class="small ok jcont" data-id="${j.id}">Pokračovat</button>` : '', !['running'].includes(j.status) ? `<button class="small danger jdel" data-id="${j.id}" title="smazat">✕</button>` : ''].filter(Boolean).join(' ')}</td></tr>`; }).join('')}
    ${state.jobs.length ? '' : '<tr><td colspan="6" class="empty">Zatím žádný upgrade. V seznamu zařízení zaškrtni zařízení a dej „Upgradovat vybrané“, nebo klikni na ▶ Upgradovat u řádku.</td></tr>'}</tbody></table></div></div>
...
function jobBanner(job, items) {
  ...
  if (job.status === 'running') return { cls: 'info', html: `<b>Probíhá${o.dry_run || /kontrola/.test(job.status_note || '') ? ' kontrola' : ' upgrade'}.</b> ${cur ? `Teď: <b>${esc(devLabel(cur))}</b> — ${esc(cur.step || 'připojení')}.` : ''} Zbývá ${c.pending || 0}. Stránku můžeš zavřít, běží to na serveru.` };
...
function renderJobDetail() {
  ...
  el.innerHTML = `<div class="panel"><h2>${esc(job.name)} ${jobBadge({ ...job, counts: Object.fromEntries(['done', 'failed', 'unknown', 'blocked', 'skipped'].map(k => [k, items.filter(i => i.status === k).length])) })}</h2>
    ...
    <div class="tablewrap"><table class="cards"><thead><tr><th>#</th><th>Zařízení</th><th>Stav</th>${adv ? '<th>Krok</th>' : ''}<th>Verze</th>${adv ? '<th>Firmware</th>' : ''}<th>Poznámka</th><th></th></tr></thead><tbody>
    ${items.map((it, i) => `<tr><td class="cardnum">${i + 1}${it.plan && it.plan.canary ? ' 🐤' : ''}</td><td class="clickable detail cardhead" data-id="${it.device_id}"><b>${esc(devLabel(it))}</b><div class="muted mono" style="font-size:11px">${esc(it.host)} · ${esc(it.board_name)}</div></td><td data-l="stav">${badge(ITEM_LABEL, it.status)}${!adv && (ACTIVE.has(it.status) || it.status === 'pending') && it.step ? `<div class="${/^přeskočí/.test(it.step) ? 'b-warn' : 'muted'}" style="font-size:11px;white-space:normal;max-width:260px">${esc(it.step)}</div>` : ''}</td>${adv ? `<td data-l="krok" style="white-space:normal">${esc(it.step)}</td>` : ''}
      <td class="mono" data-l="verze">${esc(it.from_version || it.dev_version || '')}${it.to_version && it.to_version !== it.from_version ? ` → <b>${esc(it.to_version)}</b>` : ''}</td>${adv ? `<td class="mono" data-l="firmware">${esc(it.from_fw || '')}${it.to_fw && it.to_fw !== it.from_fw ? ` → ${esc(it.to_fw)}` : ''}</td>` : ''}
      <td data-l="poznámka" style="white-space:normal;max-width:${adv ? 420 : 520}px">${it.error ? `<span style="color:var(--err)" title="${esc(it.error)}">${esc(adv ? it.error : shortReason(it.error))}</span>` : ''}${(it.warnings || []).length ? (adv ? it.warnings.map(w => `<div style="color:var(--warn);font-size:12px">⚠ ${esc(w)}</div>`).join('') : `<details class="warns"><summary>⚠ ${it.warnings.length} upozornění</summary>${it.warnings.map(w => `<div style="color:var(--warn);font-size:12px">${esc(w)}</div>`).join('')}</details>`) : ''}</td>
      <td class="acts">${[['pending'].includes(it.status) && !(rs && rs.itemId === it.id) ? `<button class="small iskip" data-id="${it.id}">přeskočit</button>` : '', ['failed', 'blocked', 'skipped', 'unknown', 'done'].includes(it.status) && !isCur ? `<button class="small iretry" data-id="${it.id}">znovu</button>` : ''].filter(Boolean).join(' ')}</td></tr>`).join('')}
    </tbody></table></div>
...
function renderHelp(m) {
  ...
  <p><b>Po restartu:</b> verze, balíčky, log, rozhraní a IP, bezdrátové spoje (stanice na stejném AP, sektoru se vrátí aspoň 80 % klientů, přičemž klienti přepojení na sousední sektor se stejným SSID se počítají jako vrácení; při kontrole radaru DFS se čeká déle; 60 GHz MCS ≥ 1 a kvalita/chybovost až po 90 s ustálení spoje z nejlepšího vzorku, CAP registrován), ping na bránu ze zařízení, sousedé, a počty položek v hlavních menu proti stavu před upgradem. Pokles je varování s odkazem na export v zálohách.</p></div>

  <div class="panel help"><h2>Časté otázky</h2>
  <details><summary>Zařízení už má u sebe jiný uživatel — jak si ho převzít</summary><p>Každé zařízení má jednoho vlastníka. Když je pod APčkem oblasti, kde jsi v userdb správce (SO) nebo zástupce (ZSO), ukáže se ve výsledku „Natáhnout z userdb“ tlačítko <b>Převzít k sobě</b>: zařízení přejdou pod tvůj účet a dosavadní vlastník je přestane vidět (stejně si je může vzít zpět). Před převzetím se navíc ověří živě na sektoru: stanice musí být právě registrovaná na sektoru tvé oblasti (u drátových kusů musí nadřazený prvek patřit do tvé oblasti); co neprojde, se vypíše s důvodem. Zařízení jiných oblastí převzít nejde; to řeší správce hromadným předáním.</p></details>
  <details><summary>Zařízení má stav „nedostupné“</summary><p>Když v chybě stojí „poškozený host key zařízení (KEY_EXCHANGE_FAILED)“, zařízení má rozbitý SSH klíč (v jeho logu „Corrupt host's key, regenerating it! Reboot required!“, typicky staré kusy); oprava je <code>/ip ssh regenerate-host-key</code> a restart. Pokud na zařízení jede telnet, udělá to tlačítko „Opravit SSH klíč přes telnet“ v detailu zařízení, uložený otisk klíče se smaže a za pár minut dej Zkontrolovat. Server se na něj nedostal přes SSH. Zkontroluj, že žije a má SSH zapnuté (IP → Services → ssh). Pak <b>Zkontrolovat</b>. Pokud má zařízení brute-force ochranu SSH (address-list), přidej IP serveru do výjimky: ta ochrana počítá nová spojení, ne špatná hesla.</p></details>
  ...
  <details><summary>„Změnil se SSH klíč, ověř zařízení“</summary><p>Zařízení se hlásí jiným klíčem než minule. Buď byl přeinstalovaný (Netinstall, výměna kusu), nebo se na té IP ozývá něco jiného. Když víš, že je to v pořádku, dej v tužce ✎ <b>reset SSH host key</b>.</p></details>
  ...
  <details><summary>„country × sektor“ / „frequency-mode × sektor“ — stanice se po upgradu nenaladí</summary><p>Skutečná příčina mrtvých antén po upgradu (9. 9. 2026): stanice měla jiné <b>country</b> nebo <b>frequency-mode</b> než sektor, po upgradu RouterOS regulaci vynutí a k sektoru naladěnému na frekvenci mimo „zemi“ stanice se už nepřipojí. Nástroj proto u každé stanice čte obě hodnoty živě ze sektoru (ze seznamu zařízení, nebo dohledaného přes sousedy a userdb) a porovnává je. Když se liší, rozhoduje, jestli stanice na frekvenci sektoru vůbec smí: <b>superchannel</b> / <b>debug</b> / country <b>Superchannel</b> pouští vše, jinak se frekvence sektoru zkontroluje proti rozsahům country stanice (ČR/ETSI 5150–5350 a 5470–5725 MHz, United States navíc 5725–5850). Smí-li, je to jen varování a upgrade projde; nesmí-li (nebo to nejde rozhodnout), položka se blokuje — sjednoť nastavení na stanici (Wireless → country, frequency-mode) a spusť znovu. <b>Na ax (ovladač wifi) „debug“ ani frequency-mode nejsou</b>; ekvivalent je country „Superchannel“, případně country, které frekvenci sektoru povoluje. Když sektor nejde dohledat (není v seznamu, nemá SSH), je to jen varování — ověř ručně.</p></details>
  <details><summary>60 GHz kusy (nRAY, Cube, wAP 60G, LHG 60G) jdou na 7.20+ po krocích</summary><p>Z verzí 7.12 až 7.17 přímo na 7.20+ tyhle kusy často nenabootují (zkušenost z fóra 9/2026: nRAY z 7.13.x končil netinstallem, po krocích 7.14.2 → 7.16.2 → 7.18.2 → cíl prošly všechny). Nástroj u nich plán skládá po těchto krocích automaticky, i po mezikroku 7.12.1 z v6. Každý krok = restart a ověření spoje.</p></details>
  <details><summary>„v7 se na tenhle HW nenahrává“ / cíl přepnut na v6</summary><p>Taková zařízení nástroj sám cílí na poslední v6 long-term. Kdo v7 chce i tak, zaškrtne u zařízení (tužka ✎) „povolit v7 i přes doporučení“ (zobrazí se jen u dotčených kusů); u MIPS-LE to nejde vůbec. Naopak kanál „v6 long-term“ u zařízení (tužka ✎) drží ten jeden kus na šestce; kdo chce na v6 nechat všechna svoje zařízení, zaškrtne v Moje nastavení „zůstat na v6“ — zařízení na RouterOS 6 pak jdou jen na poslední v6 long-term, ať mají kanál jakýkoli, a v seznamu se ukazují s kanálem v6 long-term. Zařízení už na v7 se dál aktualizují v rámci v7. Když už takový kus na v7 je (někdo mu ji nahrál ručně), nástroj ho v rámci v7 dál neupgraduje, protože každá novější v7 je těžší; doporučuje ruční návrat na 6.49.21, nebo výslovné povolení u zařízení.</p></details>
  ...
  <details><summary>„cizí balíčky .npk v kořeni“, „vadné bloky“, „device-mode flagged“</summary><p>Cizí .npk by se při restartu nainstaloval, smaž ho ve Winboxu (Files). Vadné bloky nad 5 % znamenají umírající NAND, nejdřív Netinstall nebo výměna. „Flagged“ je příznak, že RouterOS detekoval kompromitaci (SSH zranitelnosti 9/2026); zařízení prověř a příznak zruš. Flagged znamená, že RouterOS při startu našel podezřelou konfiguraci (typicky pozůstatky botnetu ze staré zranitelnosti Winboxu: scheduler s fetch, cizí L2TP/SOCKS) a vypnul ji. Nástroj takové zařízení přeskočí. Postup: projít konfiguraci a smazat cizí věci, změnit hesla, a pak buď <code>/system/device-mode/update flagged=no</code> s fyzickým potvrzením (reset nebo odpojení napájení do 5 min, klidně rukou člena), nebo u zařízení v editaci zaškrtnout „prověřeno, upgradovat i tak“. Pozor: na RouterOS 6 instalace ve flagged projde, RouterOS 7 ji odmítá („cannot install … router in flagged state“), tam je fyzické potvrzení nutné.</p></details>
  ...
  <details><summary>Co dělá „Rozdělit flash na 2 oddíly“ v detailu zařízení</summary><p>U větších zařízení (128 MB flash a víc) vytvoří záložní oddíl. Pokud pak nová verze nenabootuje, zařízení samo naběhne ze záložního oddílu se starou verzí. Jednorázová akce s restartem, dělej ji mimo špičku. Jen správce.</p></details>
  </div>
...
function settingsFields(s, global) {
  ...
  // vlastní nastavení: pole ukazuje jen vlastní volbu (prázdné = společný cíl); spodní mez = společné připnutí, horní = co nabízí MikroTik
  const floor = (state.latest && state.latest.floor) || {};
  const myPinField = (k, track, label) => { const own = String((state.settingsOwn || {})[k] || ''); const fl = floor[track] || ''; const m = mt[track] ? mt[track].version : ''; const cur = lv[track] ? lv[track].version : '';
    return `<label>${label}<input name="${k}" type="text" value="${esc(own)}" placeholder="společný cíl${fl ? ` ${fl}` : cur ? ` (${cur})` : ''}" list="pins-${track}"><datalist id="pins-${track}">${((state.latest && state.latest.known && state.latest.known[track]) || []).filter(r => !fl || cmpVer(r.version, fl) >= 0).map(r => `<option value="${esc(r.version)}">`).join('')}</datalist><span class="hint">${fl ? `nejméně ${esc(fl)} (společné minimum)` : 'bez společného minima'}${m ? `, nejvýš ${esc(m)} (MikroTik)` : ''}${own ? ` — teď vlastní ${esc(own)}` : ''}</span></label>`; };
  ...
  return `<h2>Kontroly před upgradem</h2>
    ...
    ${c('old_small_flash_split', '16 MB flash na v6 pod 6.45: nahrát zapnuté balíčky jednotlivě (system, wireless, dhcp…) místo bundle — menší zápis do flash, podle zkušeností z fóra bezpečnější než bundle; supout .rif se před nahráním smažou')}
    ${c('allow_v7_low_ram', 'v7 na zařízeních s méně než 64 MB RAM (RB750, hAP lite — hrozí OOM bootloop)')}
    ${c('allow_ap_v7_before_clients', 'sektor na v7 dřív než jeho klienti (registrovaní klienti ještě na v6, zvlášť station-bridge/nv2, se po v7 sektoru nemusí připojit — sektor se odřízne a objíždějí se členové)')}
    <h2>Služby zařízení (/ip service)</h2>
...
function renderSettings(m, adminMode = false) {
  ...
  <div class="panel"><details ${state.advanced ? 'open' : ''}><summary><b>Jak to funguje</b> (podrobně)</summary><ul class="plain">
    ...
    <li>Balíčky (.npk) stahuje server z download.mikrotik.com, ověří velikost, nahraje přes SFTP a znovu ověří na zařízení (název + velikost, žádný cizí .npk). Až potom restart.</li>
    ...
    <li>Při chybě se job <b>zastaví</b> (stop při chybě), nahrané balíčky se ze zařízení smažou, žádný restart. Blokátory (málo místa, cizí .npk, nízký uptime, změněný SSH klíč, dynamický routing při 6→7…) upgrade vůbec nespustí.</li>
    ...
    <li><b>Firewall na routerech:</b> nástroj se připojuje z IP serveru, na kterém běží — je vidět vlevo dole („SSH z …“, kliknutím se zkopíruje). Pokud mají zařízení brute-force ochranu SSH (address-list ssh_blacklist apod.), doporučuji tuto IP přidat do výjimky — nástroj sice rozestupuje opakovaná spojení na 65 s, ale sken + job dělají několik přihlášení za sebou.</li>
  ...
  const sm = $('#setf-mine'); if (sm) sm.onsubmit = async (e) => { e.preventDefault(); try { const before = state.settings.default_track; const r = await api('/settings/mine', { method: 'PUT', body: formBody(e.target) }); state.settings = r.settings; state.settingsOwn = r.own; if (r.latest) state.latest = { ...r.latest, floor: (state.latest || {}).floor }; toast('moje nastavení uloženo');
    // změna kanálu platí jen pro nově přidaná zařízení — nabídnout přepnutí i stávajících (jinak lidé čekají, že se přepnou samy)
  ...
  on('#setreset', async () => { if (!confirm('Zahodit vlastní hodnoty a používat společné nastavení?')) return; try { const r = await api('/settings/mine', { method: 'DELETE' }); state.settings = r.settings; state.settingsOwn = r.own; if (r.latest) state.latest = { ...r.latest, floor: (state.latest || {}).floor }; toast('používáš společné nastavení'); render(); } catch (e2) { toast(e2.message, true); } });
...
try { document.addEventListener('click', async (e) => { const b = e.target && e.target.closest && e.target.closest('#udbtakeover'); if (!b || !state.discovery || !(state.discovery.takeover || []).length) return; const t = state.discovery.takeover; if (!confirm(`Převzít k sobě ${t.length} zařízení (${[...new Set(t.map(x => x.owner))].join(', ')})? Dosavadní vlastník je přestane vidět.`)) return; b.disabled = true; try { const r = await api('/userdb/takeover', { method: 'POST', body: { ids: t.map(x => x.id) } }); toast(`převzato ${r.taken} zařízení${r.skipped.length ? `, ${r.skipped.length} přeskočeno` : ''}`); if (r.skipped.length) alert('Nepřevzato:\n' + r.skipped.slice(0, 30).join('\n') + (r.skipped.length > 30 ? `\n… +${r.skipped.length - 30}` : '')); state.discovery.takeover = []; await loadState(); render(); const rr = $('#discres'); if (rr) rr.innerHTML = discoveryHtml(state.discovery); } catch (e2) { toast(e2.message, true); b.disabled = false; } }); } catch {}
try { document.addEventListener('click', async (e) => { const b = e.target && e.target.closest && e.target.closest('#fixssh'); if (!b || !state.modal || state.modal.type !== 'detail') return; const d = state.modal.data.device; if (!confirm(`Přes telnet přegenerovat SSH klíč na ${devLabel(d)} (${d.host}) a zařízení restartovat?`)) return; b.disabled = true; try { await api(`/devices/${d.id}/fix-sshkey`, { method: 'POST', body: {} }); toast('SSH klíč přegenerován, zařízení se restartuje — za 2–3 minuty dej Zkontrolovat'); closeModal(); await loadState(); render(); } catch (e2) { toast(e2.message, true); b.disabled = false; } }); } catch {}
// zastavení hromadné kontroly (tlačítko v progress baru; bar se kreslí i tickerem, proto delegace na document)
try { document.addEventListener('click', async (e) => { const b = e.target && e.target.closest && e.target.closest('#checkcancel'); if (!b) return; b.disabled = true; try { const r = await api('/scan/cancel', { method: 'POST', body: {} }); if (!r.cancelled) toast('žádná kontrola neběží'); } catch (e2) { toast(e2.message, true); } }); } catch {}
...
function renderModal() {
  ...
  if (md.type === 'move') {
    ...
    if (udbOn) {
      ...
      const load = async () => {
        ...
        $('#udbgo').onclick = async () => {
          ...
          try {
            state.scanProg = null;
            await api('/userdb/import' + forUser, { method: 'POST', body: { aps, all: allMode, owner_me: !!($('#udbme') && $('#udbme').checked), track: ($('#udbtrack') || {}).value || undefined } });
    ...
    bg.innerHTML = `<div class="modal"><h2>Upravit ${esc(devLabel(d))}</h2><form id="editf" class="form">
      ...
      <label class="check"><input type="checkbox" name="skip_link_check" ${d.skip_link_check ? 'checked' : ''}> neřešit bezdrátové spoje <span class="muted">(nekontrolovat verze RouterOS klientů, country/frequency-mode, signál ani druhý konec spoje; po restartu nečekat na návrat klientů sektoru, registraci k AP, 60 GHz protějšek ani CAP — jen že zařízení naběhlo)</span></label>
      ${(d.flags && d.flags.device_mode && d.flags.device_mode.flagged) || d.ignore_flagged ? `<label class="check"><input type="checkbox" name="ignore_flagged" ${d.ignore_flagged ? 'checked' : ''}> device-mode je flagged: zařízení jsem prověřil a uklidil, upgradovat i tak <span class="muted">(funguje jen na RouterOS 6; sedmička ve flagged instalaci odmítá, tam je nutné fyzické odblokování)</span></label>` : ''}
    ...
    $('#editf').onsubmit = async (e) => { e.preventDefault(); const b = Object.fromEntries(new FormData(e.target)); b.enabled = !!b.enabled; b.allow_v7 = !!b.allow_v7; b.ignore_poe = !!b.ignore_poe; b.ignore_flagged = !!b.ignore_flagged; b.skip_link_check = !!b.skip_link_check; { const was = b.stay_v6_init === '1', now = !!b.stay_v6; if (now !== was) b.track = now ? 'v6-long-term' : 'v7-stable'; else if (!adv && now) b.track = 'v6-long-term'; } delete b.stay_v6; delete b.stay_v6_init; b.parent_id = +b.parent_id; b.port = +b.port; b.priority = +b.priority; if ('owner_id' in b) b.owner_id = +b.owner_id; if (!b.password) delete b.password; try { await api(`/devices/${d.id}`, { method: 'PUT', body: b }); toast('uloženo'); closeModal(); await loadState(); render(); } catch (e2) { toast(e2.message, true); } };
    ...
    const me = (state.auth.user || {}).id; const ownerU = (state.users || []).find(x => x.id === d.owner_id);
    bg.innerHTML = `<div class="modal"><h2>${esc(devLabel(d))} <span class="badge ${vs.cls}">${esc(vs.txt)}</span></h2>
      ${d.owner_id && me && d.owner_id !== me ? `<div class="banner warn">👤 Zařízení má u sebe jiný uživatel: <b>${esc(ownerU ? (ownerU.userdb_nick || ownerU.name) : '#' + d.owner_id)}</b>. Upgrade mu spouští on. Převzít ho jde přes „Natáhnout z userdb“ → „Převzít k sobě“ (musí být pod APčkem tvé oblasti)${state.admin ? ', nebo jako správce tužkou ✎ v poli vlastník' : ''}.</div>` : ''}
      ${sshKeyBroken ? `<div class="banner err">Zařízení má poškozený SSH host key (v jeho logu „Corrupt host's key, regenerating it! Reboot required!“). Oprava = <code>/ip ssh regenerate-host-key</code> a restart. Když na zařízení jede telnet, udělá to tlačítko: <button class="small" id="fixssh">🔧 Opravit SSH klíč přes telnet a restartovat</button></div>` : ''}
      <div class="grid2"><div>
    ...
    bg.innerHTML = `<div class="modal"><h2>Upgrade — ${devs.length} zařízení</h2>
      ...
      <form id="jobf" class="form"><label class="wide">název<input name="name" value="${esc(defaultJobName(devs))}"></label>
        ...
        <label>režim<select name="mode"><option value="upload">upload — server nahraje balíčky přes SFTP (doporučeno)</option><option value="router">router — vlastní updater zařízení (potřebuje internet)</option></select></label>
...
function checkProgHtml() {
  ...
  if (p.finishedAt) { if (now - p.finishedAt > 60000) return ''; return `<div class="wide" id="checkprog" style="flex-basis:100%"><span class="muted">${p.cancelled ? `✕ kontrola zrušena po ${p.done} z ${p.total} zařízení` : `✔ kontrola ${p.total} zařízení hotová za ${fmt(el)}`}</span><div class="progress big"><div style="width:${p.cancelled ? Math.round(p.done / p.total * 100) : 100}%"></div></div></div>`; }
  const eta = p.done ? (el / p.done) * (p.total - p.done) : 0;
  return `<div class="wide" id="checkprog" style="flex-basis:100%"><span class="muted">⟳ kontroluji zařízení: <b>${p.done}/${p.total}</b> · ${fmt(el)}${p.done ? ` · zbývá ~${fmt(eta)}` : ' · odhad po prvních výsledcích'} <button class="small" id="checkcancel" title="rozdělaná zařízení se dokončí, další se nezačnou">✕ zastavit</button></span><div class="progress big"><div style="width:${Math.round(p.done / p.total * 100)}%"></div></div></div>`;
}
...
function discoveryHtml(st) {
  ...
  st = { found: [], authFailed: [], errors: [], foreign: [], notRouterOS: [], winboxOnly: [], takeover: [], ...st };
  ...
  return `<div>${running ? '⏳ běží' : '✔ hotovo'}${st.label ? ` (${esc(st.label)})` : ''}: ${st.done}/${st.total} adres, ${st.open} s otevřeným SSH, <b>${st.added} nových založeno</b>, ${st.existing} už v seznamu, ${st.foreign.length ? `<b style="color:var(--err)">${st.foreign.length} u jiného uživatele</b>, ` : ''}${st.authFailed.length} bez platného loginu, ${st.notRouterOS.length ? `${st.notRouterOS.length} není RouterOS, ` : ''}${st.errors.length} chyb</div>
    ...
    ${st.foreign.length ? `<details open><summary>už má u sebe jiný uživatel (${st.foreign.length})</summary><div class="mono" >${st.foreign.map(esc).join('<br>')}</div>${st.takeover.length ? `<div class="row" style="margin-top:8px"><button class="small" id="udbtakeover" title="zařízení pod APčky tvé oblasti si můžeš vzít k sobě — ostatní správci a zástupci téže oblasti mají stejné právo">⇄ Převzít k sobě zařízení z mé oblasti (${st.takeover.length})</button><span class="hint">${esc([...new Set(st.takeover.map(t => t.owner))].join(', '))} — ${state.admin ? 'jako správce nástroje bez ověřování' : 'jen zařízení pod APčky, kde jsi správce nebo zástupce; stanice se ověří na sektoru'}</span></div>` : ''}</details>` : ''}
...
function renderSoon() { if (renderT) return; renderT = setTimeout(() => { renderT = null; if (!state.modal) renderLive(); }, 2000); }
...
async function loadState({ light = false } = {}) {
  // i účet a vazba na userdb: po přihlášení heslem se stránka nenačítá znovu a state.auth by zůstal z doby před přihlášením (15.9.2026: bez „Natáhnout z userdb“ a bez štítků cizích zařízení);
  // přidělená APčka (userdb.aps) se mění ve Správě za běhu
  try { const w = await api('/whoami'); if (w && w.serverStartedAt) { state.auth.serverStartedAt = w.serverStartedAt; state.auth.draining = !!w.draining; state.auth.drainJobs = w.drainJobs || []; if (w.sourceIp) state.auth.sourceIp = w.sourceIp; } if (w && w.authed) { state.auth.user = w.user || state.auth.user; state.auth.userdb = w.userdb || state.auth.userdb; state.auth.passwordLogin = w.passwordLogin; state.auth.registration = w.registration; } } catch {}
...
function connectSSE() {
  ...
  es.onmessage = (e) => {
    ...
    // překreslit jen když je zařízení v právě zobrazeném seznamu (správce s filtrem na sebe by jinak při kontrole cizích zařízení překresloval 2000 řádků každou sekundu)
    if (ev.type === 'device') { const i = state.devices.findIndex(d => d.id === ev.device.id); const wasNew = i < 0; if (i >= 0) state.devices[i] = { ...ev.device, suggested_parent: state.devices[i].suggested_parent }; else state.devices.push(ev.device); state.scanning = state.scanning.filter(x => x !== ev.device.id); if (state.view === 'devices' && (wasNew || !state.renderedIds || state.renderedIds.has(ev.device.id))) renderSoon(); }
    ...
    else if (ev.type === 'job' && ev.job) { const i = state.jobs.findIndex(j => j.id === ev.job.id); if (i >= 0) state.jobs[i] = ev.job; else state.jobs.unshift(ev.job); if (state.job && state.job.job.id === ev.job.id) { state.job.job = { ...state.job.job, ...ev.job }; } if (state.view === 'jobs') render(); else scheduleReload(false); }
...
(async () => {
  try { const w = await api('/whoami'); state.authed = w.authed; state.netHint = w.netHint || '192.0.2'; state.auth = { build: w.build || null, sso: w.sso, passwordLogin: w.passwordLogin, registration: w.registration, user: w.user, userdb: w.userdb || { enabled: false }, serverStartedAt: w.serverStartedAt || 0, sourceIp: w.sourceIp || (window.MTU_LOCAL || {}).sourceIp || '', draining: !!w.draining, drainJobs: w.drainJobs || [] }; } catch { state.authed = false; }
