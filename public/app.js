...
function renderModal() {
  ...
  if (md.type === 'move') {
    ...
    bg.innerHTML = `<div class="modal"><h2>Upravit ${esc(d.name || d.identity || d.host)}</h2><form id="editf" class="form">
      ...
      ${adv ? `<label>track<select name="track">${state.tracks.map(t => `<option ${t === d.track ? 'selected' : ''}>${t}</option>`).join('')}</select></label>` : `<input type="hidden" name="track" value="${esc(d.track)}"><label class="check"><input type="checkbox" name="stay_v6" ${d.track === 'v6-long-term' ? 'checked' : ''}> zůstat na v6 (neupgradovat na sedmičku, jen poslední 6.49.x)</label>`}
      ...
      <label class="check"><input type="checkbox" name="allow_v7" ${d.allow_v7 ? 'checked' : ''}> povolit v7 i přes doporučení (32 MB RAM, staré RB4xx…) — na vlastní riziko</label>
    ...
    $('#editf').onsubmit = async (e) => { e.preventDefault(); const b = Object.fromEntries(new FormData(e.target)); b.enabled = !!b.enabled; b.managed = !b.unmanaged; delete b.unmanaged; b.allow_v7 = !!b.allow_v7; if ('stay_v6' in b || !adv) { if (b.stay_v6) b.track = 'v6-long-term'; else if (b.track === 'v6-long-term') b.track = 'v7-stable'; } delete b.stay_v6; b.parent_id = +b.parent_id; b.port = +b.port; b.priority = +b.priority; if ('owner_id' in b) b.owner_id = +b.owner_id; if (!b.password) delete b.password; try { await api(`/devices/${d.id}`, { method: 'PUT', body: b }); toast('uloženo'); closeModal(); await loadState(); render(); } catch (e2) { toast(e2.message, true); } };
