...
function renderDevices(m) {
  ...
  m.innerHTML = `<h1>Zařízení</h1><div class="fleet"><div class="head"><div class="count">${devs.length}<small>zařízení ve správě</small></div>
      <div class="row">${toUpgrade.length && state.admin ? `<span class="hint" title="správce vidí celou síť — hromadné tlačítko tu není, aby se omylem nespustil upgrade všech zařízení; použij výběr zaškrtávátky a „Upgradovat vybrané“">k upgradu: ${toUpgrade.length}</span>` : toUpgrade.length ? `<button class="primary" id="upall">▶ Upgradovat vše potřebné (${toUpgrade.length})</button>` : `<span class="hint">${devs.length ? 'Všechna dostupná zařízení jsou aktuální.' : 'Začni přidáním zařízení.'}</span>`}</div></div>
...
function renderHelp(m) {
  ...
  <ol>
    ...
    <li><b>Spusť upgrade.</b> <b>Upgradovat vše potřebné</b> nahoře (správce ho nemá, aby omylem nespustil celou síť — vybírá zaškrtávátky), zaškrtnutá zařízení a <b>Upgradovat vybrané</b>, nebo ▶ u řádku. V dialogu můžeš zadat <b>spustit v</b> (datum a čas; pro dnešní noc vyber zítřejší datum). Bez času se jede hned.</li>
    ...
    if (udbOn) {
      ...
      const load = async () => {
        ...
        $('#udbgo').onclick = async () => {
          ...
          try {
            state.scanProg = null;
            await api('/userdb/import' + forUser, { method: 'POST', body: { aps, all: allMode, track: ($('#udbtrack') || {}).value || undefined } });
