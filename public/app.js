...
function renderModal() {
  ...
  if (md.type === 'move') {
    ...
    bg.innerHTML = `<div class="modal"><h2>Upravit ${esc(devLabel(d))}</h2><form id="editf" class="form">
      ...
      <label class="check"><input type="checkbox" name="skip_link_check" ${d.skip_link_check ? 'checked' : ''}> neověřovat bezdrátové spoje po restartu <span class="muted">(nečekat na návrat klientů sektoru, registraci stanice k AP, 60 GHz protějšek ani CAP — jen že zařízení naběhlo)</span></label>
