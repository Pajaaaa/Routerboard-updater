...
- **Import z userdb (hkfree):** účet navázaný na správce oblasti v userdb (SSO podle e-mailu, nebo ručně správcem v Uživatelích) si tlačítkem „Načíst z userdb“ naimportuje zařízení svých APček i antény členů pod nimi včetně loginů (`/monitoring/get-zarizeni?ap=&uzivatele=1` + `/device/get-credentials/<ip>`). Typy zařízení z userdb se ignorují, co je RouterOS rozhodne sken po SSH. APčko = skupina, antény členů mají štítek „člen“. Klíč v `MTU_USERDB_USER/PASS`. Nástroj `node tools/userdb-who.js <uid|e-mail|nick> [--devices]` ukáže, komu co patří.
